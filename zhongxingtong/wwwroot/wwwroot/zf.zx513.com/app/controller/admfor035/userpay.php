...
class userpay extends CheckAdmin
{
    ...
    public function savepay()
    {
		...
		if ($id && $data && $data['is_state'] == '4') {
			...
			$userst = $this->model()->select()->from('users')->where(array('fields' => 'id=?', 'values' => array($iid)))->fetchRow();

		
			$unpaid=	$userst['unpaid']+$data['money'];
			$paid=	$userst['paid']-$data['money'];

			$datat['unpaid']	=	$unpaid;
			$datat['paid']		=	$paid;

			$this->model()->from('users')->updateSet($datat)->where(array('fields' => 'id=?', 'values' => array($iid)))->update();
