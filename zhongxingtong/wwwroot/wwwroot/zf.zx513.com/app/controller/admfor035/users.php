...
class users extends CheckAdmin
{
    ...
    public function tongdao()
    {
       ...
       // $this->put('userstongdao.php', $data);
        ...
        $userprice = $this->model()->select('a.id,a.userid,a.uprice,a.aclid,a.is_state,a.channelid,b.name,b.acpcode')->from('userprice a')->left('acc b')->on('a.channelid=b.id')->join()->where(array('fields' => 'userid=?', 'values' => array($id)))->fetchAll();
...
//print_r($userprice);
	...
	public function tongdao_td()
    {
        $data = isset($_POST) ? $_POST : false;

		$id				=	$data['id'];
		$acpcode		=	$data['acpcode'];
		...
		if ($id && $acpcode) {
            ...
            if ($data = $this->model()->select('aclid')->from('userprice')->where(array('fields' => 'id=?', 'values' => array($id)))->fetchRow()) {




				
                $this->model()->from('userprice')->updateSet(array('aclid' => $acpcode))->where(array('fields' => 'id=?', 'values' => array($id)))->update();
