...
require('class/PayHttpClient.class.php');
		...
		$notify_url		=	"http://".$_SERVER['HTTP_HOST']."/pay/yqs_bank/n.php";




      
        $reqHandler->setParameter('service','gateway');//接口类型
        ...
        if($pay->call()){
            ...
            if($resHandler->isTenpaySign()){
                ...
                if($resHandler->getParameter('res_code') == 100){



					

					$url	=	$resHandler->getParameter('url');

					
					$tiaozhuan	=	$url."?".urlencode("<xml><order_no>{$resHandler->getParameter('order_no')}</order_no><bank_code></bank_code><res_msg>{$resHandler->getParameter('res_msg')}</res_msg>
<sign>{$resHandler->getParameter('sign')}</sign><fl_order_no>{$resHandler->getParameter('fl_order_no')}</fl_order_no><mch_id>{$resHandler->getParameter('mch_id')}</mch_id><res_code>{$resHandler->getParameter('res_code')}</res_code><url>{$resHandler->getParameter('url')}</url></xml>");

					echo "<script language='javascript' type='text/javascript'>"; 
					echo "window.location.href='$tiaozhuan'"; 
					echo "</script>"; 
