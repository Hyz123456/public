...
class apisubmit extends api
{
    ...
    protected function submit()
    {
		...
		$acc = $this->model()->select('a.id,a.acpcode,a.gateway,a.acwid,a.is_state,b.is_state as is_state_acc,b.channelid,b.aclid')->from('acc a')->left('userprice b')->on('b.channelid=a.id')->join()->where(array('fields' => 'b.userid=? and a.acwid=?', 'values' => array($customerid, $acw['id'])))->fetchRow();
		...
		$name = $this->model()->select()->from('acl')->where(array('fields' => 'acwid=? and acpcode=?', 'values' => array($acwid,$aclid)))->fetchRow();
		...
		if ($paytype	==	"alipay"	||	$paytype	=="weixin"){
			...
			include 'lunxun/inc.php';

			if ($kaiqi=="1"){

			$zhanghao	=		get_zh($data);
			$shuju		=		get_xx($zhanghao,$data,$bdata);
			$accounts	=		$shuju[0];
		...
		$orderdata = array('userid' => $customerid, 'accounts' => $accounts, 'agentid' => $this->userData['superid'], 'orderid' => $orderid, 'acpcode' => $acpcode, 'sdorderno' => $sdorderno, 'total_fee' => $total_fee, 'channelid' => $channelid, 'addtime' => $addtime, 'lastime' => $addtime, 'is_paytype' => 0, 'orderinfoid' => $orderinfoid);
		...
		//$url='http://zf.xbyy168.com/pay/fmt_kj/send.php?orderid=2018020112301781723&price=5.00&bankcode=kuaijie&remark=';
	//echo $url;
		//exit;
