...
class Handleorder extends Model
{
	...
	public function add_xieyi($yinhang,$contractId,$orderNo)
    {


		$orderdata = array('yinhang' => $yinhang, 'xieyi' => $contractId, 'dingdan' => $orderNo);

		$xieyi	=	$this->model("xieyi")->select()->from('xieyi')->where(array('fields' => 'yinhang=?', 'values' => array($yinhang)))->fetchRow();



		if ($xieyi){

			return $xieyi["dingdan"];

		}else{

			if (!($orid = $this->model()->from('xieyi')->insertData($orderdata)->insert())) {
