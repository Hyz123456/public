...
if (1==1) {
   ...
   /* $dataArray = [
        ...
        'pmt_tag' => 'weixin', //支付类型,目前支持微信和qq，微信传weixin,qq传qq，必传
    ...
    $dataArray = [
        ...
        'notify_url' => "http://".$_SERVER['HTTP_HOST']."/pay/zyf_wap/n.php", //异步通知URL，一定外网地址才能收到回调通知信息，必传
        'sync_url' => 'http://www.baidu.com',
        'body' => 'Wap支付', //订单描述，必传
        'pay_type' => 'weixin', //支付类型,目前支持微信，微信传weixin  必传

		'client_ip' => $iipp, //支付类型,目前支持微信，微信传weixin  必传
	...
	if (isset($array['data']['out_pay_url'])){

		$url	=	$array['data']['out_pay_url'];
		Header("Location: $url"); 
