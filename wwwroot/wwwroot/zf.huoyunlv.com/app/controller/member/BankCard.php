...
class BankCard extends CheckUser
{
	...
	//add
		...
		if ($this->userData['is_verify_phone'] == '1') { //是否认证
			...
			if(strlen($Code)>1 and $Code!=$_SESSION["code"]){
				...
				if ($data['phone'] == '' || $data['realname'] == '' || $data['idcard'] == '' || $data['batype'] == '' || $data['baname'] == '' || $data['baaddr'] == '' ) {
				echo json_encode(array('status' => 0, 'msg' => '选项填写不完整'));
			...
			//}
			$where = array('fields' => 'baname=?', 'values' => array($data['baname']));
            $userinfo = $this->model()->select()->from('userinfo')->where($where)->fetchRow();
			
			$data['userid']= $_SESSION['login_userid'];
			if($userinfo){
				echo json_encode(array('status'=>0,'msg'=>'不能添加同一张银行卡'));
				exit;
			}elseif($this->model()->from('userinfo')->insertData($data)->insert())
			{
				echo json_encode(array('status'=>1,'msg'=>'添加成功'));
				exit;
			}
			
			//$this->model()->from('cfo')->insertData($data)->insert()
