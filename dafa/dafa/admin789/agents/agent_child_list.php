...
<table width="100%" border="0" cellpadding="5" cellspacing="1" class="font12" style="margin-top:5px;" bgcolor="#CCC">
    <tr style="background-color:#5a5a5a; color:#FFF">
        <td style="width: 15%" align="center" height="25"><strong>下属会员名</strong></td>
    ...
    $sql	=	"SELECT u.id FROM agents_list a,user_list u
                WHERE a.id=".$_GET['id']." AND a.id=u.top_id AND u.top_id!=0 GROUP by u.id";
    ...
    if($bid){
        ...
        $sql_lottery	=	"SELECT SUM(IF(sub.bet_money>0,IF(sub.is_win!=2,sub.bet_money,0),0)) bet_money_total,SUM(IF(sub.is_win=1,sub.win+sub.fs,IF(is_win=0,fs,0))) win_total,u.id
                    ...
                    WHERE u.top_id=".$_GET['id']." AND u.top_id!=0 AND o.status!='0' AND o.status!='3' AND u.user_id=o.user_id AND o.order_num=sub.order_num
                    ";
        ...
        $sql_ds	=	"SELECT SUM(IF(k.bet_money>0,k.bet_money,0)) bet_money_total,SUM(IF(k.win>0,k.win,0)+IF(k.fs>0,k.fs,0)) win_total,u.id
                    ...
                    WHERE u.top_id=".$_GET['id']." AND u.top_id!=0 AND k.status!=0 AND k.status!=3 AND k.check!=0 AND u.user_id=k.user_id
                    ";
        ...
        $sql_cg	=	"SELECT SUM(IF(k.bet_money>0,k.bet_money,0)) bet_money_total,SUM(IF(k.win>0,k.win,0)+IF(k.fs>0,k.fs,0)) win_total,u.id
                    ...
                    WHERE u.top_id=".$_GET['id']." AND u.top_id!=0 AND k.status!=0 AND k.status!=3 AND k.check=1 AND u.user_id=k.user_id
                    ";
        ...
        $sql_live	=	"SELECT SUM(IF(lo.bet_money>0,lo.bet_money,0)) bet_money_total,SUM(IF(lo.live_win is not null,lo.live_win,0)) win_total,u.id
                    ...
                    WHERE u.top_id=".$_GET['id']." AND u.top_id!=0 AND l.user_id=u.user_id AND l.live_username=lo.live_username
                    ";
        ...
        foreach($list_main as $key => $value){
            ...
            <tr align="center" onMouseOver="this.style.backgroundColor='<?=$over?>'" onMouseOut="this.style.backgroundColor='<?=$out?>'" style="background-color:<?=$color?>; line-height:20px;">
                <td height="40" align="center" valign="middle"><a title="<?=$value['user_name']?>" style="color: #F37605;" href="/agent/agent_report/all_game_index.php?s_time=<?=$s_time?>&amp;e_time=<?=$e_time?>&amp;user_group=<?=$value['user_name']?>&amp;date_month=<?=$date_month?>"><?=$value['user_name']?></a></td>
