...
<tr style="background-color:#5a5a5a; color:#FFF">
    ...
    <td style="width: 15%" align="center"><strong>存款金额(排除活动金额)</strong></td>
    <td style="width: 15%" align="center"><strong>取款金额(排除活动金额)</strong></td>
    <td style="width: 14%" align="center"><strong>合计盈利(排除活动金额)</strong></td>
    <td style="width: 13%" align="center"><strong>后台加钱(用于活动)</strong></td>
    <td style="width: 13%" align="center"><strong>后台扣钱(用于活动)</strong></td>
</tr>
<?php
...
if($user_group || $user_ignore_group){
    ...
    if($userArrayString && $userIgnoreArrayString){
        $sql_sub = " WHERE agents_name IN($userArrayString) AND agents_name NOT IN($userIgnoreArrayString)";
    }elseif($userArrayString && !$userIgnoreArrayString){
        $sql_sub = " WHERE agents_name IN($userArrayString)";
    }elseif(!$userArrayString && $userIgnoreArrayString){
        $sql_sub = " WHERE agents_name NOT IN($userIgnoreArrayString)";
    }
...
$sql	=	"SELECT a.id FROM agents_list a WHERE 1=1 ";
if($inUserString != "") $sql .= " AND a.id IN $inUserString";
//    if($s_time) $sql.=" and o.bet_time>='".$s_time." 00:00:00'";
//    if($e_time) $sql.=" and o.bet_time<='".$e_time." 23:59:59'";
$sql.=" ORDER BY a.regtime DESC";
...
if($bid){
    $bid	=	rtrim($bid,',');
    $sql_main = "select id,agents_name from agents_list where id in($bid) ORDER BY regtime DESC ";
    ...
    $sql_ck	=	"SELECT SUM(m.order_value) ck_money_total, u.top_id
                    FROM agents_list a,user_list u,money m
                    WHERE a.id in($bid) AND a.id=u.top_id AND u.top_id!=0 AND u.user_id=m.user_id AND (m.type='在线支付' or m.type='后台充值') AND m.about not like '%用于活动%' AND m.status='成功'
                    ";
    if($s_time) $sql_ck.=" and m.update_time>='".$s_time." 00:00:00' ";
    if($e_time) $sql_ck.=" and m.update_time<='".$e_time." 23:59:59' ";
    $sql_ck .= " GROUP BY u.top_id ORDER BY a.regtime DESC ";
    ...
    $sql_hk	=	"SELECT SUM(m.order_value) hk_money_total, u.top_id
                    FROM agents_list a,user_list u,money m
                    WHERE a.id in($bid) AND a.id=u.top_id AND u.top_id!=0 AND u.user_id=m.user_id AND m.type='银行汇款' AND m.about not like '%用于活动%' AND m.status='成功'
                    ";
    if($s_time) $sql_hk.=" and m.update_time>='".$s_time." 00:00:00' ";
    if($e_time) $sql_hk.=" and m.update_time<='".$e_time." 23:59:59' ";
    $sql_hk .= " GROUP BY u.top_id ORDER BY a.regtime DESC ";
    ...
    $sql_qk	=	"SELECT SUM(m.order_value) qk_money_total, u.top_id
                    FROM agents_list a,user_list u,money m
                    WHERE a.id in($bid) AND a.id=u.top_id AND u.top_id!=0 AND u.user_id=m.user_id AND (m.order_value<0) AND m.about not like '%用于活动%' AND m.status='成功'
                    ";
    if($s_time) $sql_qk.=" and m.update_time>='".$s_time." 00:00:00' ";
    if($e_time) $sql_qk.=" and m.update_time<='".$e_time." 23:59:59' ";
    $sql_qk .= " GROUP BY u.top_id ORDER BY a.regtime DESC ";
    ...
    $sql_ck_hd	=	"SELECT SUM(m.order_value) ck_money_total_hd, u.top_id
                    FROM agents_list a,user_list u,money m
                    WHERE a.id in($bid) AND a.id=u.top_id AND u.top_id!=0 AND u.user_id=m.user_id AND (m.type='在线支付' or m.type='后台充值') AND m.about like '%用于活动%' AND m.status='成功'
                    ";
    if($s_time) $sql_ck_hd.=" and m.update_time>='".$s_time." 00:00:00' ";
    if($e_time) $sql_ck_hd.=" and m.update_time<='".$e_time." 23:59:59' ";
    $sql_ck_hd .= " GROUP BY u.top_id ORDER BY a.regtime DESC ";
    ...
    $sql_qk_hd	=	"SELECT SUM(m.order_value) qk_money_total_hd, u.top_id
                    FROM agents_list a,user_list u,money m
                    WHERE a.id in($bid) AND a.id=u.top_id AND u.top_id!=0 AND u.user_id=m.user_id AND (m.order_value<0) AND m.about like '%用于活动%' AND m.status='成功'
                    ";
    if($s_time) $sql_qk_hd.=" and m.update_time>='".$s_time." 00:00:00' ";
    if($e_time) $sql_qk_hd.=" and m.update_time<='".$e_time." 23:59:59' ";
    $sql_qk_hd .= " GROUP BY u.top_id ORDER BY a.regtime DESC ";
    ...
    foreach($list_main as $key => $value){
        ...
        foreach($list_ck_hd as $key4 => $value_ck_hd){
            if($value["id"] == $value_ck_hd["top_id"]){
        ...
        foreach($list_qk_hd as $key5 => $value_qk_hd){
            if($value["id"] == $value_qk_hd["top_id"]){
        ...
        <tr align="center" onMouseOver="this.style.backgroundColor='<?=$over?>'" onMouseOut="this.style.backgroundColor='<?=$out?>'" style="background-color:<?=$color?>; line-height:20px;">
            <td height="40" align="center" valign="middle"><a title="<?=$value['agents_name']?>" style="color: #F37605;" href="agent_child_list_inout.php?id=<?=urldecode($value['id'])?>&amp;angent_name=<?=$value['agents_name']?>&amp;s_time=<?=$s_time?>&amp;e_time=<?=$e_time?>&amp;date_month=<?=$date_month?>"><?=$value['agents_name']?></a></td>
