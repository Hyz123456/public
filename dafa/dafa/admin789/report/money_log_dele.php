...
if($act=='delete'){
	...
	if($stdate&&$eddate){
		...
		if($mysql->error){
			echo "<script>alert('$mysqli->error'); location.href='/admin/report/money_log_dele.php'</script>";
		}
		$res = $mysqli->affected_rows;
		if($res>=0){
			echo "<script>alert('删除记录 $res 条 !'); location.href='/admin/report/money_log_dele.php'</script>";
		}else{
			echo "<script>alert('删除失败 !');  location.href='/admin/report/money_log_dele.php'</script>";
		}
		exit;
