        ...
        <tr>
            <td valign="top">
                ...
                <table width="100%" border="0" cellpadding="5" cellspacing="1" class="font12" style="margin-top:5px;" bgcolor="#CCC">
                    ...
                    $sql	=	"select distinct u.id  FROM user_list u,agbetdetail a where a.playerName=";
					if($_GET['gtype']=='AG'){
						$sql.="u.ag_username and a.platformType in('ag','agin') ";
					}
					else if($_GET['gtype']=='BBIN'){
						$sql.="u.bb_username and a.platformType in('bb','bbin') ";
					}
					else if($_GET['gtype']=='MG'){
						$sql.="u.mg_username and a.platformType in('mg','mgin') ";
					}
					elseif($_GET['gtype']=='NA'){
							$sql.="u.na_username and a.platformType in('na','nain') ";
						}
						elseif($_GET['gtype']=='PT'){
							$sql.="u.pt_username and a.platformType in('pt','ptin') ";
						}
						elseif($_GET['gtype']=='ALLBET'){
							$sql.="u.ab_username and a.platformType in('allbet','allbetin') ";
						}
                    ...
                    if($bid){
                        ...
                        $sql	=	"SELECT user_name,pay_name,user_id,
                                              count(a.id) bet_count,sum(betAmount) bet_money_total,
                                              SUM(validBetAmount) bet_money_total_valid,
                                              SUM(netAmount) win_total
                        ...
                        while ($rows = $query->fetch_array()) {
                            ...
                            <tr align="center" onMouseOver="this.style.backgroundColor='<?=$over?>'" onMouseOut="this.style.backgroundColor='<?=$out?>'" style="background-color:<?=$color?>; line-height:20px;">
                                <td height="40" align="center" valign="middle"><?=$_GET["gtype"]?></td>
                                <td align="center" valign="middle"><a title="<?=$rows['user_name']?>" style="color: #F37605;" href="live_user.php?gtype=<?=$_GET["gtype"]?>&amp;s_time=<?=$s_time?>&amp;e_time=<?=$e_time?>&amp;user_id=<?=$rows['user_id']?>&amp;date_month=<?=$date_month?>&amp;user_group=<?=$rows['user_name']?>&amp;user_ignore_group=<?=$user_ignore_group?>"><?=$rows['user_name']?></a>(<?=$rows['pay_name']?>)</td>
                    ...
                    <tr style="background-color:#FFFFFF;">
                        <td colspan="7" align="center" valign="middle">当前页总投注金额:<?=$t_allmoney?>元 &nbsp;&nbsp;   当前页投注结果:<?=$t_sy?>元&nbsp;&nbsp;   当前页赢取金额:<?=$t_allmoney - $t_sy?>元 &nbsp;&nbsp;   如果是正数，说明赢钱，如果是负数，则为输钱。</td>
                    </tr>
                    <tr style="background-color:#FFFFFF;">
                        <td colspan="7s" align="center" valign="middle"><?php echo $pageStr;?></td>
                    </tr>
