                ...
                <table width="100%" border="0" cellpadding="5" cellspacing="1" class="font12" style="margin-top:5px;" bgcolor="#CCC">
                    ...
                    if($bid){
                        ...
                        $sql	=	"SELECT id,user_id,order_num,cg_count, bet_money, bet_win, win,
                                              bet_time, bet_time_et, status, balance, assets, www, fs
                                      FROM k_bet_cg_group
                                      WHERE id in($bid)
                                      order by id desc";
                        $query	=	$mysqli->query($sql);
                        ...
                        while ($rows = $query->fetch_array()) {
                            ...
                            <tr align="center" onMouseOver="this.style.backgroundColor='<?=$over?>'" onMouseOut="this.style.backgroundColor='<?=$out?>'" style="background-color:<?=$color?>; line-height:20px;">
                                ...
                                <td><div><div style="float:left;">已结算：<?=$yjs[$rows["gid"]]?>&nbsp;[<a style="color: #F37605;" href="../sports/bet_cg.php?gid=<?=$rows["id"]?>&order_num=<?=$rows["order_num"]?>">详细</a>]</div><div style="float:right;"><span style="color:#999999;"><?=$rows["assets"]?></span>&nbsp;<?=$row_sub["user_name"]?>&nbsp;<span style="color:#999999;"><?=$rows["balance"]?></span></div><div style="float:left; padding-left:10px;"><?=$rows['www']?></div></div></td>
                                <td align="center" valign="middle"><?=$rows['bet_money']?></td>
                                <td align="center" valign="middle"><?=$rows['fs']?></td>
                                <td align="center"><?=$rows["win"]+$rows['fs']?></td>
                    ...
                    <tr style="background-color:#FFFFFF;">
                        <td colspan="8" align="center" valign="middle">当前页总投注金额:<?=$t_allmoney?>元 &nbsp;&nbsp;   当前页投注结果:<?=$t_sy?>元&nbsp;&nbsp;   当前页赢取金额:<?=$t_allmoney - $t_sy?>元</td>
                    </tr>
                    <tr style="background-color:#FFFFFF;">
