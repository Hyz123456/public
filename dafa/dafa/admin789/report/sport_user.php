        ...
        <tr>
            <td valign="top">
                    ...
                    <form name="form1" id="form1" method="get" action="<?=$_SERVER["REQUEST_URI"]?>" onSubmit="return check();">
                        <tr>
                            <td align="left" bgcolor="#FFFFFF">
                                <a title="返回上一页" style="color: #F37605;"  href="sport_history.php?s_time=<?=$s_time?>&amp;e_time=<?=$e_time?>&amp;user_group=<?=$user_group?>&amp;user_ignore_group=<?=$user_ignore_group?>&amp;date_month=<?=$date_month?>">返回上一页</a>
                            </td>
                        </tr>
                        <tr>
                            <td align="left" bgcolor="#FFFFFF">
                ...
                <table width="100%" border="0" cellpadding="5" cellspacing="1" class="font12" style="margin-top:5px;" bgcolor="#CCC">
                    ...
                    $sql	=	"select k.user_id  FROM user_list u,k_bet k
                                where u.user_id=k.user_id AND k.bet_money>0 ";
                    if($inUserString != "") $sql .= " AND k.user_id IN $inUserString";
                    if($s_time) $sql.=" and k.bet_time>='".$s_time." 00:00:00'";
                    if($e_time) $sql.=" and k.bet_time<='".$e_time." 23:59:59'";
                    ...
                    if($bid){
                        ...
                        $sql	=	"SELECT u.user_name,u.pay_name,u.user_id,
                                              count(k.id) bet_count,sum(k.bet_money) bet_money_total,
                                              SUM(CASE WHEN k.status!=0 AND k.status!=3 AND k.`check`!=0   THEN  k.bet_money  ELSE  0 END) bet_money_total_valid,
                                              SUM(IF(k.win>0,k.win,0)+IF(k.fs>0,k.fs,0)) win_total
                                      FROM user_list u,k_bet k
                                      WHERE u.user_id in($bid) AND u.user_id=k.user_id
                                      ";
                        if($inUserString != "") $sql .= " AND k.user_id IN $inUserString";
                        if($s_time) $sql.=" and k.bet_time>='".$s_time." 00:00:00'";
                        if($e_time) $sql.=" and k.bet_time<='".$e_time." 23:59:59'";
                        ...
                        $sql.=" group by k.user_id order by k.id desc";
                        $query	=	$mysqli->query($sql);
...
//                            }
                            ...
                            <tr align="center" onMouseOver="this.style.backgroundColor='<?=$over?>'" onMouseOut="this.style.backgroundColor='<?=$out?>'" style="background-color:<?=$color?>; line-height:20px;">
                                ...
                                <td align="center" valign="middle"><a title="<?=$rows['user_name']?>" style="color: #F37605;" href="sport_details.php?gtype=<?=urldecode($gtype)?>&amp;s_time=<?=$s_time?>&amp;e_time=<?=$e_time?>&amp;date_month=<?=$date_month?>&amp;user_group=<?=$rows['user_name']?>&amp;user_ignore_group=<?=$user_ignore_group?>"><?=$rows['user_name']?></a>(<?=$rows['pay_name']?>)</td>
