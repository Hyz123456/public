...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#CCCCCC">
    <tr>
        <td height="24" nowrap class="bg_tr"><font ><span class="STYLE2">汇款管理：查看所有的用户汇款信息</span></font></td>
    </tr>
    <tr>
                ...
                <form name="form1" method="GET" action="huikuan.php" >
                    <tr>
                        <td width="124" align="center"><select name="status" id="status">
                                ...
                                <option value="失败" <?=$_GET["status"]=='失败' ? 'selected' : ''?> style="color:#FF0000;">汇款失败</option>
                                <option value="成功" <?=$_GET["status"]=='成功' ? 'selected' : ''?> style="color:#006600;">汇款成功</option>
                                <option value="全部汇款" <?=$_GET["status"]=='全部汇款' ? 'selected' : ''?>>全部汇款</option>
                            </select></td>
                        <td width="124" align="center"><select name="order" id="order">
                                <option value="id" <?=$_GET["order"]=='id' ? 'selected' : ''?>>默认排序</option>
                                <option value="order_value" <?=$_GET["order"]=='order_value' ? 'selected' : ''?>>汇款金额</option>
                                <option value="zsjr" <?=$_GET["order"]=='zsjr' ? 'selected' : ''?>>赠送金额</option>
                            </select></td>
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#CCCCCC">
    <tr>
        <td height="24" nowrap bgcolor="#FFFFFF">
            ...
            <table width="100%" border="1" bgcolor="#FFFFFF" bordercolor="#96B697" cellspacing="0" cellpadding="0" style="border-collapse: collapse; color: #225d9c;" id=editProduct   idth="100%" >
                <tr bgcolor="efe" class="bg_tr" align="center">
                    <th height="24" >编号</th>
                    <th>订单号 / 提交时间</th>
                    <th><a href="?order=order_value">充值金额</a></th>
                    <th>赠送金额</th>
                    <th>查看财务</th>
                    <th>汇款银行</th>
                    <th>汇款方式 / 汇款地址</th>
                ...
                $sql		=	"select id from money where order_value>=0 and type='银行汇款' and deleted=0";                           //0422
                if(isset($_GET["status"])){
                    if($_GET["status"]!="全部汇款") $sql .=	" and status='".$_GET["status"]."'";
                }
                if($_GET["username"]){
                    $sql	.=	" and order_num like('"."%\_".$_GET["username"]."')";
                }
                if($_GET["time_start"]){
                ...
                if($id){
                    $id		=	rtrim($id,',');
                    $arr	=	array();
                    $sql	=	"select * from money where id in ($id) and deleted=0 order by $order desc";       //0422
                    $query	=	$mysqli->query($sql);
                    while($rows = $query->fetch_array()){
                        $m_sum +=	$rows["order_value"];
                 ...
                 ?>
                        <tr align="center" onMouseOver="this.style.backgroundColor='#EBEBEB'" onMouseOut="this.style.backgroundColor='#ffffff'" >
                            ...
                            <td><a href="hk_look.php?id=<?=$rows["id"]?>"><?=$rows["order_num"]?></a><br/><?=$rows["update_time"]?></td>
                            <td><span style="color:#999999;"><?=$rows["assets"]?></span><br /><?=$rows["order_value"]?><br /><span style="color:#999999;"><?=$rows["balance"]?></span></td>
                            <td><?=$rows["zsjr"]?></td>
                            <td><a href="hccw.php?username=<?=$arr[1]?>&status=成功" target="_blank">查看财务</a></td>
                            <td><a href="<?=$_SERVER["REQUEST_URI"]?>&bank=<?=$rows["pay_card"]?>"><?=$rows["pay_card"]?>
                                </a></td>
                            <td><?=$rows["manner"]?><br/><?=$rows["pay_address"]?></td>
                            <td><?php
                                if($rows["status"]=="成功"){
                                    $t_sum += $rows["order_value"];
                                    echo '<span style="color:#006600;">汇款成功</span>';
                                }else if($rows["status"]=="失败"){
                                    $f_sum += $rows["order_value"];
                                    echo '<span style="color:#FF0000;">汇款失败</span>';
                                }else{
                                    $c_sum += $rows["order_value"];
                                    echo '<a href="hk_look.php?id='.$rows["id"].'"><span style="color:#FF9900;">审核中(结算)</span></a>';
                                }
                                ?></td>
                        </tr>
                    <?php
    ...
    <tr>
        <td><div>总金额：<span style="color:#0000FF"><?=$m_sum?></span>，成功：<span style="color:#006600"><?=$t_sum?></span>，赠送金额：<span style="color:#FF00FF"><?=$zs_sum?></span>，失败：<span style="color:#FF0000"><?=$f_sum?></span>，审核：<span style="color:#FF9900"><?=$c_sum?></span></div>
