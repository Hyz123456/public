...
//if($_GET["action2"]=="del"){   
    $sql_del="delete from manage_log where id='".$_GET['action3']."' ";
...
//$id=implode(",",$_GET['action3']);
	...
	$sql_delall=" delete from manage_log where id ='".$val."' ";
...
<input type="button" onclick="delAll()" value="一键删除" style="margin:4px;">
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#CCCCCC">
  <tr>
		...
		$color	= "#FFFFFF";
      	?>
			...
			<td align="center"><input type="checkbox" name="delete"value="<?=$rows["id"]?>"></td>
			  ...
			  <td align="center"><a href="log.php?action3=<?=$rows["id"]?>">删除</a></td>
          </tr> 	
      	<?
      }
}
