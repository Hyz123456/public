...
function check(){
	...
	if(num){
		...
		if(action=="0"){
			...
			if(action=="2") document.form1.action="ft_list.php?type=tennis_match&php=wqbf";
			if(action=="4") document.form1.action="ft_nullity.php?type=tennis_match&php=wqbf";
		}
	}else{
        alert("您未选中任何复选框");
        return false;
...
?>
        ...
        <td width="200"><a href="wqbf_yjs.php?date=<?=$page_date?>" style="font-size:13px;">&gt;&gt;进入已结算</a></td>
        <td width="200" align="center"><label>
          <input type="button" name="Submit" value="刷新网球" onClick="window.location.reload();">
        </label></td>
		<td width="200"><a href="wqbf.php?date=<?=$page_date?>&look=1" style="font-size:13px;">&gt;&gt;只看有下注的赛事</a></td>
  ...
  <table   border="0" cellspacing="1" cellpadding="5"  bgcolor="006255" width="100%" height="41">
    ...
    <?php
	$sql		=	"SELECT ID,Match_ID,Match_Time,Match_Master,Match_Guest,Match_Name,MB_Inball,TG_Inball FROM tennis_match where match_js=0 and match_date='$page_date' order by Match_CoverDate,match_name,Match_Master desc";
