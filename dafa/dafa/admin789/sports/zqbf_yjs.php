...
function check(){
	...
	if(num){
		...
		if(action=="2") document.form1.action="ft_list.php?type=bet_match&php=zqbf_yjs";
		if(action=="1") document.form1.action="re_jiesuan.php?type=bet_match&php=zqbf_yjs";
		if(action=="3") document.form1.action="set_bet_score.php";
	}else{
        alert("您未选中任何复选框");
        return false;
    }
...
?>
        ...
        <td width="200"><a href="zqbf.php?date=<?=$page_date?>" style="font-size:13px;">&lt;&lt;返回未结算</a></td>
        <td width="200" align="center"><input type="button" name="Submit" value="刷新足球" onClick="window.location.reload();"></td>
		<td width="200"><a href="zqbf_yjs.php?date=<?=$page_date?>&look=1" style="font-size:13px;">&gt;&gt;只看有下注的赛事</a></td>
  ...
  <table   border="0" cellspacing="1" cellpadding="0"  bgcolor="006255" width="100%" height="41">
    <tr class="m_title_ft"> 
    ...
    <?php
   $sql			=	"SELECT Match_ID,Match_Time,Match_Master,Match_Guest,Match_Name,MB_Inball,TG_Inball,MB_Inball_HR,TG_Inball_HR FROM Bet_Match where match_js=1 and (match_Date='$page_date' or match_date='$page_date2') order by  Match_CoverDate,match_name,Match_Master desc";
    ...
    <tr class="m_cen" align="center" onMouseOver="this.style.backgroundColor='#EBEBEB'" onMouseOut="this.style.backgroundColor='#FFFFFF'"> 
	  ...
	  <td width="50"><div align="left" style="padding-left:5px;"><?if($cou>0){?><a href="orderlist.php?status=all&type=足球&match_id=<?=$rows["Match_ID"]?>"><?=$cou?></a><?}else{echo '0';}?></div></td>
      <td width="100"> 
