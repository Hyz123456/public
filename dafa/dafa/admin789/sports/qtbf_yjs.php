...
function check(){
	...
	if(num){
		...
		if(action=="2") document.form1.action="ft_list.php?type=other_match&php=qtbf_yjs";
		if(action=="1") document.form1.action="re_jiesuan.php?type=other_match&php=qtbf_yjs";
		if(action=="3") document.form1.action="set_wpb_score.php?type=other_match&php=qtbf_yjs";
	}else{
        alert("您未选中任何复选框");
        return false;
    }
...
?>
        ...
        <td width="200"><a href="qtbf.php?date=<?=$page_date?>" style="font-size:13px;">&lt;&lt;返回未结算</a></td>
        <td width="200" align="center"><input type="button" name="Submit" value="刷新其他赛事" onClick="window.location.reload();"></td>
		<td width="200"><a href="qtbf_yjs.php?date=<?=$page_date?>&look=1" style="font-size:13px;">&gt;&gt;只看有下注的赛事</a></td>
  ...
  <table   border="0" cellspacing="1" cellpadding="0"  bgcolor="006255" width="900" height="41">
    ...
    <?php
    $sql		=	"SELECT Match_ID,Match_Time,Match_Master,Match_Guest,Match_Name,MB_Inball,TG_Inball FROM other_match where match_js=1 and match_Date='$page_date' order by  Match_CoverDate,match_name,Match_Master desc";
