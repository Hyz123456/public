...
function check(){
	...
	if(num){
		...
		if(action=="0"){
			...
			if(action=="2") document.form1.action="ft_list.php?type=baseball_match&php=bqbf";
			if(action=="4") document.form1.action="ft_nullity.php?type=baseball_match&php=bqbf";
		}
	}else{
        alert("您未选中任何复选框");
        return false;
...
?>
        ...
        <td width="200"><a href="bqbf_yjs.php?date=<?=$page_date?>" style="font-size:13px;">&gt;&gt;进入已结算</a></td>
        <td width="200" align="center"><label>
          <input type="button" name="Submit" value="刷新棒球" onClick="window.location.reload();">
        </label></td>
		<td width="200"><a href="bqbf.php?date=<?=$page_date?>&look=1" style="font-size:13px;">&gt;&gt;只看有下注的赛事</a></td>
  ...
  <table   border="0" cellspacing="1" cellpadding="5"  bgcolor="006255" width="100%" height="41">
    <tr class="m_title_ft"> 
    ...
    <?php
	$sql		=	"SELECT ID,Match_ID, Match_Date, Match_Time, Match_Master, Match_Guest,Match_Name,MB_Inball,TG_Inball,MB_Inball_HR,TG_Inball_HR FROM baseball_match where match_js=0 and match_date='$page_date' order by  Match_CoverDate,match_name,Match_Master desc";
    ...
    <tr style="background-color:#ffffff" align="center" onMouseOver="this.style.backgroundColor='#EBEBEB'" onMouseOut="this.style.backgroundColor='#ffffff'"> 
	  ...
	  <td width="50"><div align="left" style="padding-left:5px;"><?if($cou>0){?><a href="orderlist.php?status=all&type=棒球&match_id=<?=$rows["Match_ID"]?>"><?=$cou?></a><?}else{echo '0';}?></div></td>
     <td width="100"> 
        <input name="<?="mh".$ftid;?>" type="text" class="zqinput" id="mih<?=$rows["Match_ID"]?>" onChange="zqlrbf_sb(<?=$rows["Match_ID"]?>,<?=$ftid?>)" value="<?=$rows["MB_Inball_HR"]?>"  style="width:30px; text-align:center;" maxlength="3" />
        &nbsp;&nbsp;
        <input name="<?="th".$ftid;?>" type="text" class="zqinput" id="tih<?=$rows["Match_ID"]?>" onChange="zqlrbf_sb(<?=$rows["Match_ID"]?>,<?=$ftid?>)" value="<?=$rows["TG_Inball_HR"]?>" style="width:30px; text-align:center;" maxlength="3" />
      </td>
     ...
     <td width="32"><input name="mid[]" type="checkbox" id="mid[]" value="<?=$rows["Match_ID"]?>" onClick="zqlrbf(<?=$rows["Match_ID"]?>,<?=$ftid?>)" /></td> 
    </tr>
    <?php 
	}
	?>
