...
if($_GET["action"]=="re"){
    ...
    if(count($_GET['bid'])>0){
		foreach($_GET['bid'] as $i=>$bid){
			...
			admin::insert_log($_SESSION["login_name"],get_ip(),$bj_time_now,"重新结算了单号为[".$order_num."]的上半场注单",session_id(),"",$bj_time_now);
		}
	}
	//串关结算开始
	if(count($_GET['bid_cg'])>0){
		foreach($_GET['bid_cg'] as $i=>$bid){
			...
			$rows	 =	$query->fetch_array();
			$order_num=$rows["order_num"];
			admin::insert_log($_SESSION["login_name"],get_ip(),$bj_time_now,"重新结算了单号为[".$order_num."]的上半场串关注单",session_id(),"",$bj_time_now);
		}
	}
	//赛事重新结算
	...
	if($_GET["bid"] && $_GET["bid_cg"])
	{
		...
		admin::insert_log($_SESSION["login_name"],get_ip(),$bj_time_now,"手工重新结算足球,单式[".$ds_str."],串关[".$cg_str."]",session_id(),"",$bj_time_now);
	}
...
$sql		=	"select k_bet.*,user_list.user_name from k_bet left join user_list on k_bet.user_id=user_list.user_id where k_bet.lose_ok=1 and k_bet.status!=0 and match_id in($mid) and (k_bet.point_column in ('match_bmdy','match_bgdy','match_bhdy','match_bho','match_bao','match_bdpl','match_bxpl') or k_bet.point_column like 'match_hr_bd%' ) order by  k_bet.id desc"; //单式
$result		=	$mysqli->query($sql);

$sql		=	"select k_bet_cg.*,user_list.user_name,k_bet_cg_group.win from k_bet_cg left join user_list on k_bet_cg.user_id=user_list.user_id left join k_bet_cg_group on k_bet_cg_group.id=k_bet_cg.gid where match_id in($mid) and k_bet_cg.status!=0 and match_id in($mid) and(ball_sort like('%上半场%') or bet_info like('%上半场%')) order by k_bet_cg.id desc";
