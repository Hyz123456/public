...
?>
  ...
  <table width="100%"   height="41" border="0" cellpadding="5" cellspacing="1"  bgcolor="006255" >
    <tr class="m_title_ft"> 
      <th height="24" align="middle"><?=$page_date?></th>
      <th align="middle">时间</th>
      <th align="middle">主场队伍</th>
      <th align="middle">全场分</th>
      <th align="middle">客场队伍</th>
	  <th align="middle">单式注单数</th>
	  <th align="middle">串关注单数</th>
      <th align="middle">第一节</th>
      <th align="middle">第二节</th>
      <th align="middle">第三节</th>
      <th align="middle">第四节</th>
      <th align="middle">上半场</th>
      <th align="middle">下半场</th>
      <th align="middle">加时</th>
      <th align="middle">结算比分</th>
      <th align="middle"><input name="checkall" type="checkbox" id="checkall" onClick="return ckall();"/></th>
    </tr>
	<?php
	...
	?>
      ...
      <td height="18" align="middle"><?=$rows["Match_ID"]?>
        <br/>
        <?=$rows["Match_Name"]?></td>
      <td align="middle"><?=$rows["Match_Time"]?></td>
      <td align="middle"><?=$rows["Match_Master"]?></td>
      <td align="middle"><span name="div<?=$ftid?>MB_Inball" id="div<?=$rows["Match_ID"]?>MB_Inball"><?=$rows["MB_Inball"]?></span>
        <br />
        <span name="div<?=$ftid?>TG_Inball" id="div<?=$rows["Match_ID"]?>TG_Inball"><?=$rows["TG_Inball"]?></span></td>
      <td align="middle"><?=$rows["Match_Guest"]?></td>
	  <td><div align="middle" style="padding-left:5px;"><?if($cou>0){?><a href="orderlist.php?status=all&type=篮球&match_id=<?=$rows["Match_ID"]?>&s_time=<?=date('Y-m-d',strtotime('-13 day'))?>"><?=$cou?></a><?}else{echo '0';}?></div></td>
	  <td><div align="middle" style="padding-left:5px;"><?if($cou_cg>0){?><a href="cg_list.php?status=all&type=篮球&gid_group=<?=$gid_group?>&s_time=<?=date('Y-m-d',strtotime('-13 day'))?>"><?=$cou_cg?></a><?}else{echo '0';}?></div></td>
      <td align="middle"><span name="div<?=$ftid?>MB_Inball_1st" id="div<?=$rows["Match_ID"]?>MB_Inball_1st"><?=$rows["MB_Inball_1st"]?></span>
        <br />
        <span name="div<?=$ftid?>TG_Inball_1st" id="div<?=$rows["Match_ID"]?>TG_Inball_1st"><?=$rows["TG_Inball_1st"]?></span></td>
      <td align="middle"><span name="div<?=$ftid?>MB_Inball_2st" id="div<?=$rows["Match_ID"]?>MB_Inball_2st"><?=$rows["MB_Inball_2st"]?></span>
        <br />
        <span name="div<?=$ftid?>TG_Inball_2st" id="div<?=$rows["Match_ID"]?>TG_Inball_2st"><?=$rows["TG_Inball_2st"]?></span></td>
      <td align="middle"><span name="div<?=$ftid?>MB_Inball_3st" id="div<?=$rows["Match_ID"]?>MB_Inball_3st"><?=$rows["MB_Inball_3st"]?></span>
        <br />
        <span name="div<?=$ftid?>TG_Inball_3st" id="div<?=$rows["Match_ID"]?>TG_Inball_3st"><?=$rows["TG_Inball_3st"]?></span></td>
      <td align="middle"><span name="div<?=$ftid?>MB_Inball_4st" id="div<?=$rows["Match_ID"]?>MB_Inball_4st"><?=$rows["MB_Inball_4st"]?></span>
        <br />
        <span name="div<?=$ftid?>TG_Inball_4st" id="div<?=$rows["Match_ID"]?>TG_Inball_4st"><?=$rows["TG_Inball_4st"]?></span></td>
      <td align="middle"><span name="div<?=$ftid?>MB_Inball_HR" id="div<?=$rows["Match_ID"]?>MB_Inball_HR"><?=$rows["MB_Inball_HR"]?></span>
        <br />
        <span name="div<?=$ftid?>TG_Inball_HR" id="div<?=$rows["Match_ID"]?>TG_Inball_HR"><?=$rows["TG_Inball_HR"]?></span></td>
      <td align="middle"><span name="div<?=$ftid?>MB_Inball_ER" id="div<?=$rows["Match_ID"]?>MB_Inball_ER"><?=$rows["MB_Inball_ER"]?></span>
        <br />
        <span name="div<?=$ftid?>TG_Inball_ER" id="div<?=$rows["Match_ID"]?>TG_Inball_ER"><?=$rows["TG_Inball_ER"]?></span></td>
      <td align="middle"><span name="div<?=$ftid?>MB_Inball_Add" id="div<?=$rows["Match_ID"]?>MB_Inball_Add"><? if ($rows["MB_Inball_Add"]>0) echo $rows["MB_Inball_Add"]; ?></span>
        <br />
      ...
      <td align="middle"><input name="mid[]" type="checkbox" id="mid[]" value="<?=$rows["Match_ID"]?>" onClick="zqlrbf(<?=$rows["Match_ID"]?>,<?=$ftid?>)" /></td>
    </tr>
<?php
	}
	unset($arr_bet);
