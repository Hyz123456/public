...
if(@$_GET["action"]=="save"){
	$uid	=	intval($_GET["uid"]);
	$sql	=	"update user_list set user_pass='".md5($_POST["password"])."',user_pass_naked='".$_POST["password"]."' where user_id=$uid";
...
if(@$_GET["action"]=="saveqk"){
	$uid=intval($_GET["uid"]);
	$sql="update user_list set qk_pass='".md5($_POST["qk_pwd"])."' where user_id=$uid";
	$mysqli->autocommit(FALSE);
	$mysqli->query("BEGIN"); //事务开始
	try{
		$mysqli->query($sql);
...
<?php
$sql	=	"select user_id,user_name from user_list where user_id=".intval($_POST["uid"][0])." limit 1";
...
<table width="100%" border="1" bgcolor="#FFFFFF" bordercolor="#96B697" cellspacing="0" cellpadding="0" style="border-collapse: collapse; color: #225d9c;" id=editProduct   idth="100%" >
  ...
  <tr>
    <td height="24" align="center" nowrap bgcolor="#FFFFFF"><form action="set_pwd.php?action=save&uid=<?=$rows["user_id"]?>" method="post"  name="form1" onSubmit="return check_sub();">
      <p>&nbsp;</p>
  ...
  <tr>
    <td height="24" align="center" nowrap bgcolor="#FFFFFF"><form action="set_pwd.php?action=saveqk&uid=<?=$rows["user_id"]?>" method="post" name="form1" onSubmit="return check_sub1();">
      <p>&nbsp;</p>
      <table width="661" border="1" align="center" bordercolor="#333333"  style="border-collapse:collapse;color:#000;">
  <tr>
    <td bgcolor="#F0FFFF">用户名</td>
...
?>
  <tr>
    <td height="24" align="center" nowrap bgcolor="#FFFFFF"><form action="set_pwd.php?action=savedl&uid=<?=$rows["user_id"]?>" method="post"  name="form1" onSubmit="return check_sub2();">
      <p>&nbsp;</p>
      <table width="661" border="1" align="center" bordercolor="#333333"  style="border-collapse:collapse;color:#000;">
  <tr>
    <td bgcolor="#F0FFFF">用户名</td>
