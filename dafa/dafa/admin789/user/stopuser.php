...
if($go == 1){ //停用
	...
	$sql = "select * from user_list where user_id in ($uid)";
    $query=$mysqli->query($sql);
	$arr=array();
	while($row_u=$query->fetch_array()){
		$arr[]=$row_u;
	}
	foreach($arr as $k=>$v){
		$sql="select id from agents_list where agents_name='".$v['user_name']."'";
		$query=$mysqli->query($sql);
		$row_c=$query->fetch_array();
		$sql = "select id from agents_list where agentsname='".$v['pay_name']."'";
		$query	=	$mysqli->query($sql);
		$row2= $query->fetch_array();
		if($v['pay_num']){
			$sql = "select id from agents_list where bank='".$v['pay_num']."'";
			$query	=	$mysqli->query($sql);
			$row3= $query->fetch_array();
		}
		if($row_c['id'] || $row2['id'] || $row3['id']){
			echo "<script  type ='text/javascript'>alert('已经有代理绑定了该用户名或者真实姓名或者银行卡号！！！');history.go(-1);</script>";
			exit;
		}
		$sql1= "insert into agents_list(agents_name,agents_pass,agent_pass_naked,qk_pass,regtime,tel,qq,othercon,remark,agentsname,agents_add,bank)
            values('".$v['user_name']."','".$v['user_pass']."','".$v['user_pass_naked']."','".$v['qk_pass']."',now(),'".$v['tel']."','".$v['qq']."','".$v['othercon']."','".$v['remark']."','".$v['pay_name']."','".$v['pay_bank']."','".$v['pay_num']."')";
		$mysqli->query($sql1);
	}
