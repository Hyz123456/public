...
foreach($user_array as $key => $value){
    ...
    for($i=0;$i<=$lost_days;$i++){
        ...
        $sql	=	"SELECT live_username, sum(bet_money) betAmount,SUM(IFNULL(VALIDBETAMOUNT,0)) validBetAmount
...
$sql.=" and (live_type='xin' )";
	...
	$sql.=" and live_type='dxdz' and game_room='游戏消费'";
        ...
        if($live_type=='DX'){
             $valid_money=(-$results["betAmount"]);
          }
		  else{ $valid_money = $results["validBetAmount"];}
...
$sql.=" and (live_type='xin' )";
	...
	$sql.=" and live_type='dxdz' ";
        ...
        $sql = "select count(id) COUNT_ID,SUM(FSMONEY) FS_ALREADY_MONEY from live_fs_list where USERNAME_LIVE='$username' ";
		if($live_type=="BBIN"){
    $sql.=" and live_type='".$live_type."' ";
}elseif($live_type=="BBINDZ"){
    $sql.=" and live_type='BBINDZ' ";
...
$sql.=" and (live_type='xin' )";
	...
	$sql.=" and live_type='dxdz' ";
		...
		//$fs_money=$fs_money-$results_FS_LIST['FS_ALREADY_MONEY'];
        if($results_FS_LIST["COUNT_ID"]>0 || $fs_money==0){
            ...
            $sql = "insert into live_fs_list(USERNAME_LIVE,USERNAME,VALIDMONEY,FSMONEY,FS_RATE,ADDTIME,FSTIME,live_type) values('$username','$user_name','$valid_money','$fs_money','$fs_rate',now(),'".($query_time." ".substr($insert_time,0,8))."','$live_type')";
            $query	=	$mysqli->query($sql);
            $id 	=	$mysqli->insert_id;
