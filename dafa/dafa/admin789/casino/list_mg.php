...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#CCCCCC">
  <tr>
    <td height="24" nowrap class="bg_tr"><font >&nbsp;<span class="STYLE2">MG注单管理：查看所有注单情况（注单为美东时间）</span></font></td>
  </tr>
  <tr>
...
<?php 
      $sql	=	"select a.id from agbetdetail a,user_list u where a.platformType='mg' and a.playerName=u.mg_username ";
	  ...
	  if($bid){
	  	$bid	=	rtrim($bid,',');
	  	$sql="select a.*,b.user_name from (select * from agbetdetail where id in($bid)) a left join user_list b on a.playerName = b.mg_username order by a.betTime desc";
		//echo $sql;die;
	  	$query	=	$mysqli->query($sql);
	 	...
	 	$bet_money=$win=$val_money=0;
      	...
      	?>
	        <tr align="center" onMouseOver="this.style.backgroundColor='<?=$over?>'" onMouseOut="this.style.backgroundColor='<?=$out?>'" style="background-color:<?=$color?>;">
              ...
              <td><a href="list_mg.php?username=<?=$rows["playerName"]?>&date_start=<?=$_GET['date_start']?>&date_end=<?=$_GET['date_end']?>"><?=$rows["playerName"]?></a>
			  </td>
    ...
    <tr>
      <td >
    该页统计:总注金:<?=$bet_money?>，有效投注金额:<?=$val_money?>，会员结果：<span style="color:<?=$win < 0 ? '#FF0000' : '#009900'?>;"><?=$win?></span>
  </td>
    </tr>
  <tr><td >
