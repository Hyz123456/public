...
if($_GET['live_type']=="BBIN"){
	...
	if($_GET['live_type']=="MG"){
		...
		$live_type = "PT";
	}else if($_GET['live_type']=="DX"){
		$live_type = "DX";
	}else if($_GET['live_type']=="xin"){
		$live_type = "xin";
	}
	else if($_GET['live_type']=="BBINDZ"){
		$live_type = "BBINDZ";
	}else if($_GET['live_type']=="BBINCP"){
		$live_type = "BBINCP";
	}else if($_GET['live_type']=="BBINTY"){
		$live_type = "BBINTY";
	}else{
	$live_type = "AG";
	}
...
<table width="100%" border="0" align="center" cellpadding="5" cellspacing="1" class="font12" bgcolor="#ccc">
    <tr>
        <td height="24" nowrap class="bg_tr"><font >&nbsp;<span class="STYLE2">反水列表</span></font></td>
    </tr>
        ...
        <tr>
            <td align="left" bgcolor="#FFFFFF">
				...
				<select name="live_type" id="live_type">
                    <option value="AG"  <?=$live_type=='AG' ? 'selected' : ''?>>AG真人</option>
					<option value="xin"  <?=$live_type=='xin' ? 'selected' : ''?>>AG电子</option>
                    <option value="BBIN"  <?=$live_type=='BBIN' ? 'selected' : ''?>>BBIN真人</option>
					<option value="BBINDZ"  <?=$live_type=='BBINDZ' ? 'selected' : ''?>>BBIN电子</option>
					<option value="BBINCP"  <?=$live_type=='BBINCP' ? 'selected' : ''?>>BBIN彩票</option>
					<option value="BBINTY"  <?=$live_type=='BBINTY' ? 'selected' : ''?>>BBIN体育</option>
					<option value="MG"  <?=$live_type=='MG' ? 'selected' : ''?>>MG电子</option>
					<option value="ALLBET"  <?=$live_type=='ALLBET' ? 'selected' : ''?>>ALLBET</option>
					<option value="NA"  <?=$live_type=='NA' ? 'selected' : ''?>>NA电子</option>
					<option value="PT"  <?=$live_type=='PT' ? 'selected' : ''?>>PT电子</option>
					<option value="DX"  <?=$live_type=='DX' ? 'selected' : ''?>>棋牌</option>
                </select>
                &nbsp;&nbsp;
...
<tr style="background-color:#5a5a5a; color:#FFF">
    ...
    <th>下注笔数</th>
    <th>下注额度</th>
    <th<?if($live_type=='DX'){echo 'display:none;';}?>><strong >派彩额度</th>
    <th>有效下注额度</th>
    <th>所选时间内总反水金额</th>
    <th>所选时间内需要反水金额</th>
    <th>所选时间内已反的金额</th>
    <th>反水比例<br><input id="setAllFsRate" type="button" onclick="setAllFsRate();" value="设置所有玩家" style="margin:3px 0;" /></th>
    <th>操作<br><input id="setAllFs" name="fs_button" type="button" onclick="setAllFs();" value="反水本页玩家" style="margin:3px 0;" /></th>
</tr>
<?php
include("../../include/pager.class.php");
if($live_type=='AG'){ 
$sqst="select * from live_user where live_type='AGIN'";
...
foreach($li as $va){
	$id=$va['user_id'];
  $sqt="select * from live_user where live_type='xin' and user_id='$id'";
  $xin=$mysqli->query($sqt);
  $xins = $xin->fetch_array();
  if(empty($xins)){
    $sql="insert into live_user VALUES (0,'$va[user_id]','xin','$va[live_username]','$va[live_password]',0,0,now(),'A',0,0,2)";$mysqli->query($sql); 
  
  }
}
}
if($live_type=='BBIN'){
$sqst="select * from live_user where live_type='BBIN'";
$list=$mysqli->query($sqst);
$li=array();
while($rowl = $list->fetch_array()){$li[]=$rowl;}
//var_dump($li);
foreach($li as $va){
	$id=$va['user_id'];
  $sqt="select * from live_user where live_type='BBINDZ' and user_id='$id'";
  $sqc="select * from live_user where live_type='BBINCP' and user_id='$id'";
    $sqy="select * from live_user where live_type='BBINTY' and user_id='$id'";
  $xin=$mysqli->query($sqt);
  $xins = $xin->fetch_array();
  if(empty($xins)){
    $sql="insert into live_user VALUES (0,'$va[user_id]','BBINDZ','$va[live_username]','$va[live_password]',0,0,now(),'A',0,0,2)";$mysqli->query($sql); 
  
  }
   $xin=$mysqli->query($sqc);
  $xins = $xin->fetch_array();
  if(empty($xins)){
    $sql="insert into live_user VALUES (0,'$va[user_id]','BBINCP','$va[live_username]','$va[live_password]',0,0,now(),'A',0,0,2)";$mysqli->query($sql); 
  
  }
   $xin=$mysqli->query($sqy);
  $xins = $xin->fetch_array();
  if(empty($xins)){
    $sql="insert into live_user VALUES (0,'$va[user_id]','BBINTY','$va[live_username]','$va[live_password]',0,0,now(),'A',0,0,2)";$mysqli->query($sql); 
...
if($user_group || $user_ignore_group){
    ...
    $sql .= $sql_sub;
    $query	=	$mysqli->query($sql)or die ("err!");
    $rs = array();
...
if($live_type=="BBIN"){
    $sql.=" and live_type='BBIN' and gamcde!='bbdz' and gamcde!='bbcp' and gamcde!='bbty' ";
}elseif($live_type=="BBINDZ"){
    $sql.=" and live_type='BBIN' and gamcde='bbdz' ";
}
elseif($live_type=="BBINCP"){
    $sql.=" and live_type='BBIN' and gamcde='bbcp' ";
}elseif($live_type=="BBINTY"){
    $sql.=" and live_type='BBIN' and gamcde='bbty' ";
}

elseif($live_type=="xin"){
$sql.=" and (live_type='xin' )";
}elseif($live_type=="AG"){
	...
	$sql.=" and live_type='dxdz' and game_room='游戏消费'";
}
...
if($live_type=="BBIN"){
    ...
    $sql.=" and live_type='BBINDZ'";
}
elseif($live_type=="BBINCP"){
    $sql.=" and live_type='BBINCP' ";
}elseif($live_type=="BBINTY"){
    $sql.=" and live_type='BBINTY' ";
}
...
$sql.=" and (live_type='xin' )";
	...
	$sql.=" and live_type='dxdz' ";
        ...
        if($row_user && $row_user["id"]){
            ...
            $user_id = $row_user["id"];
			if($live_type=='DX'){
			$fs_total_money =  number_format(((-$row["betAmount"]) / 100) * $fs_rate, 2, '.', '');
			}else{
...
$sql.=" and (live_type='xin' )";
	...
	$sql.=" and live_type='dx' ";
        ...
        <tr align="center" onMouseOver="this.style.backgroundColor='<?=$over?>'" onMouseOut="this.style.backgroundColor='<?=$out?>'" style="background-color:<?=$color?>; line-height:20px; <? if($row_user["user_name"]){}else{echo 'display:none;'; } ?>">
            ...
            <td align="center" valign="middle"><? if($live_type=='DX'){echo -$row['betAmount']; }else{echo $row['betAmount'];}?></td>
            <td align="center" valign="middle" style="<?if($live_type=='DX'){echo 'display:none;';}?>"><?=$row['netAmount']?></td>
            <td align="center" valign="middle"><? if($live_type=='DX'){echo -$row['betAmount']; }else{echo $row['validBetAmount'];}?></td>
