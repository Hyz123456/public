...
if($_GET['username']) {
	$sql="select dx_username from k_user where username = '" . $_GET['username'] . "'";
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#CCCCCC">
  <tr>
    <td height="24" nowrap class="bg_tr"><font >&nbsp;<span class="STYLE2">棋牌报表（注单为北京时间）</span></font></td><!-- BH电子真人报表 -->
  </tr>
  <tr>
        ...
        <td width="729" align="left">
          会员名：
			...
			<td style="position: relative;top: 17px;">
			  ...
			  <td style="position: relative;top: 17px;">
              <input data="../casino/report_bb.php"" type="Button" class="hrets" value="棋牌"></td>
...
<?php 
		$sql = "SELECT COUNT(a.id) AS cnt,SUM(if(a.betAmount<0,a.betAmount,0)) AS money,SUM(a.betAmount) AS ba,a.playerName,u.user_name FROM agbetdetail a,user_list u where a.platformType='dxdz' and a.betTime >= '".$_GET['date_start']."' and a.betTime <= '".$_GET['date_end']."' and a.playerName = u.dx_username";
		if($_GET['username']) 
			$sql .= " and a.playerName = '$user'";
		$sql .= " GROUP BY a.playerName ORDER BY cnt DESC,money desc";
		$query = $mysqli->query($sql);
	        ...
	        <tr align="center" onMouseOver="this.style.backgroundColor='<?=$over?>'" onMouseOut="this.style.backgroundColor='<?=$out?>'" style="background-color:<?=$color?>;">
			 <td  align="center" ><?php echo $rows['user_name']?></td>
	          <td  align="center" ><?php echo $rows['playerName']?></td>
			   <td><a href="javascript:window.open('/newdx/cha2.php?u=<?php echo $rows['playerName']?>','list','width=440,height=180,left=50,top=100,scrollbars=no').focus();">点击获取</a></td>
...
<?php
    ...
    <tr>
      <td >
    统计:总注金:<?=0-$bet_money?>，会员结果：<span style="color:<?=$win < 0 ? '#FF0000' : '#009900'?>;"><?=$win?></span>
  </td>
    </tr>
