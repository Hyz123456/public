...
function update_auto(){
	window.open('/newmg2/getall.php','list','width=440,height=180,left=50,top=100,scrollbars=no').focus();
}
</script>
</HEAD>
...
<?php 
		$sql = "SELECT mg_username FROM user_list where mg_username!='' ";
		...
		if($query)
      	...
      	?>
	        <tr align="center" onMouseOver="this.style.backgroundColor='<?=$over?>'" onMouseOut="this.style.backgroundColor='<?=$out?>'" style="background-color:<?=$color?>;">
			  <td  align="center" class='ag_name' ><?php echo $rows['mg_username']?></td>
