...
if($_GET['username']) {
	$sql="select na_username from user_list where user_name = '" . $_GET['username'] . "'";
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#CCCCCC">
  <tr>
    <td height="24" nowrap class="bg_tr"><font >&nbsp;<span class="STYLE2">NA真人报表（注单为美东时间）</span></font></td>
  </tr>
  <tr>
...
<?php 
		$sql = "SELECT COUNT(a.id) AS cnt, SUM(a.betAmount) AS ba, SUM(a.netAmount) AS na, SUM(a.validBetAmount) AS va,a.playerName,u.user_name FROM agbetdetail a,user_list u where a.platformType='na' and a.betTime >= '".$_GET['date_start']."' and a.betTime <= '".$_GET['date_end']."' and a.playerName = u.na_username";
		if($_GET['username']) 
	        ...
	        <tr align="center" onMouseOver="this.style.backgroundColor='<?=$over?>'" onMouseOut="this.style.backgroundColor='<?=$out?>'" style="background-color:<?=$color?>;">
			 <td  align="center" ><?php echo $rows['user_name']?></td>
	          <td  align="center" ><?php echo $rows['playerName']?></td>
			   <td><a href="javascript:window.open('/newna2/cha2.php?u=<?php echo $rows['playerName']?>','list','width=440,height=180,left=50,top=100,scrollbars=no').focus();">点击获取</a></td>
