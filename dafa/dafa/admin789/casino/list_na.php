...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#CCCCCC">
  <tr>
    <td height="24" nowrap class="bg_tr"><font >&nbsp;<span class="STYLE2">NA注单管理：查看所有注单情况（注单为美东时间）</span></font></td>
  </tr>
  <tr>
...
<?php 
      $sql	=	"select a.id from agbetdetail a,user_list u where a.platformType='na' and a.playerName=u.na_username ";
	  ...
	  if($bid){
	  	$bid	=	rtrim($bid,',');
		$sql="select a.*,b.user_name from (select * from agbetdetail where id in($bid)) a left join user_list b on a.playerName = b.na_username order by a.betTime desc";
	  	$query	=	$mysqli->query($sql);
	 	...
	 	$bet_money=$win=$val_money=0;
      	...
      	?>
	        <tr align="center" onMouseOver="this.style.backgroundColor='<?=$over?>'" onMouseOut="this.style.backgroundColor='<?=$out?>'" style="background-color:<?=$color?>;">
              ...
              <td><a href="list_na.php?username=<?=urlencode($rows["playerName"])?>&date_start=<?=$_GET['date_start']?>&date_end=<?=$_GET['date_end']?>"><?=$rows["playerName"]?></a>
			  </td>
