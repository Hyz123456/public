...
function update_from_romote(){
	...
	window.open('/newbbin2/getallRecord.php?u='+ag_users+'&d='+$('#date_start').val().split(' ')[0],'list','width=440,height=180,left=50,top=100,scrollbars=no').focus();*/
}
function update_auto(){
	window.open('/newbbin2/getall.php','list','width=440,height=180,left=50,top=100,scrollbars=no').focus();
}
</script>
</HEAD>
      ...
      <tr align="center">
        ...
        <td width="729" align="left">
          会员名：
            ...
            <input type="button" onclick="update_from_romote()" value="同步所选日期所选会员数据">
			 <input type="button" onclick="update_auto()" value="自动同步">
			</td>
...
<?php 
		$sql = "SELECT bb_username FROM user_list where bb_username!='' ";
		...
		if($query)
      	...
      	?>
	        <tr align="center" onMouseOver="this.style.backgroundColor='<?=$over?>'" onMouseOut="this.style.backgroundColor='<?=$out?>'" style="background-color:<?=$color?>;">
			  <td  align="center" class='ag_name' ><?php echo $rows['bb_username']?></td>
			  <td><input type="checkbox" class="chk_list" name="id[]" value="1"></td>
			  <td><!--a href="javascript:window.open('/newbbin2/getallRecord.php?u=<?php echo $rows['ag_username']?>'+'&d='+$('#date_start').val().split(' ')[0],'list','width=440,height=180,left=50,top=100,scrollbars=no').focus();">更新</a-->更新</td>
