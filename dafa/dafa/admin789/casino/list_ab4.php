...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#CCCCCC">
    <tr>
        <td height="24" nowrap background="../images/06.gif"><font > <span class="STYLE2">ALLBet投注记录</span></font></td>
    </tr>
    <tr>
                ...
                <form name="form1" method="get" action="<?=$_SERVER["REQUEST_URI"]?>" onSubmit="return check();">
                    <tr align="center">
                        ...
                        <td nowrap align="center"> 游戏类型：
                            <select onChange="self.form1.submit()" name="gameType" id="gameType">
                                <option value="">全部</option>
                                <option value="101" <?= $_GET['gameType']=='101' ? 'selected="selected"' : ''?>>普通百家乐</option>
                                <option value="102" <?= $_GET['gameType']=='102' ? 'selected="selected"' : ''?>>VIP百家乐</option>
                                <option value="103" <?= $_GET['gameType']=='103' ? 'selected="selected"' : ''?>>急速百家乐</option>
                                <option value="104" <?= $_GET['gameType']=='104' ? 'selected="selected"' : ''?>>竞咪百家乐</option>
                                <option value="201" <?= $_GET['gameType']=='201' ? 'selected="selected"' : ''?>>骰寶</option>
                                <option value="301" <?= $_GET['gameType']=='301' ? 'selected="selected"' : ''?>>龍虎</option>
                                <option value="401" <?= $_GET['gameType']=='401' ? 'selected="selected"' : ''?>>輪盤</option>
                            </select> </td>
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#CCCCCC"><tr><td ><table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#CCCCCC">
                <tr>
                    <td height="24" nowrap bgcolor="#FFFFFF"><table width="100%" border="1" bgcolor="#FFFFFF" bordercolor="#96B697" cellspacing="0" cellpadding="0" style="border-collapse: collapse; color: #225d9c;" id=editProduct   idth="100%" >
                            ...
                            $sql = "select id , betNum from api_abbetdetail WHERE 1=1 ";
							
							$sql2 = 'select sum(winOrLoss) as Payoff, sum(validAmount) as  Commissionable from api_abbetdetail WHERE 1=1 ';
                            ...
                            if( $_GET['gameType']!="" ) $sql .= "and gameType = '" . $_GET['gameType'] . "' ";
							 if( $_GET['gameType']!="" ) $sql2 .= "and gameType = '" . $_GET['gameType'] . "' ";
...
// 	$sql   = "select * from ag_Transfer where id in($nid) ";
                                $sql   = "select * from api_abbetdetail where id in($nid) ";
                                if($_GET['username'])$sql .="and username='".$_GET['username']."' ";
...
// 	echo $sql;
                                ...
                                while($rows = $query->fetch_array()){
                                    ...
                                    <tr onMouseOver="this.style.backgroundColor='#EBEBEB'" onMouseOut="this.style.backgroundColor='#FFFFFF'" style="background-color:#FFFFFF;">
                                        ...
                                        <td align="center" ><?=$rows["betNum"]?></td>
                                        <td align="center" ><?=$gameTypestr?></td>
                                        <td align="center" valign="middle"><?=date("Y-m-d H:i:s",strtotime($rows["betTime"]))?></td>
                                        <td align="center" ><?=$rows["tableName"]?></td>
                                        <td align="center" ><?=$rows["betType"]?></td>
                                        <td align="center" ><?=$rows["betAmount"]?></td>
                                        <td align="center" ><?=$rows["winOrLoss"]?></td>
                                        <td align="center" ><?=$rows["validAmount"]?></td>
    ...
    统计:输赢金额:<?=$countArr['Payoff']?>，有效投注:<?=$countArr['Commissionable']?></span>
                ...
                <tr>
                    <td > <?=$page->get_htmlPage("list.php?username=".$username."&t_time=".$t_time."&end_time=".$end_time."&reflag=".$reflag."&gameType=".$gameType);?></td>
                </tr>
            </table></td></tr>
</table>
</body>
