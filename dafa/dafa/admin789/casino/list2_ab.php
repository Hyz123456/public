...
function update_from_romote(){
	...
	window.open('/newallbet2/getallRecord.php?u='+ag_users+'&d='+$('#date_start').val().split(' ')[0],'list','width=440,height=180,left=50,top=100,scrollbars=no').focus();*/
}

function update_auto(){
	window.open('/newallbet2/getall.php','list','width=440,height=180,left=50,top=100,scrollbars=no').focus();
}
</script>
</HEAD>
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#CCCCCC">
  <tr>
    <td height="24" nowrap class="bg_tr"><font >&nbsp;<span class="STYLE2">ALLBET真人报表</span></font></td>
  </tr>
  <tr>
...
<?php 
		$sql = "SELECT ab_username FROM user_list where ab_username!='' ";
		...
		if($query)
      	...
      	?>
	        <tr align="center" onMouseOver="this.style.backgroundColor='<?=$over?>'" onMouseOut="this.style.backgroundColor='<?=$out?>'" style="background-color:<?=$color?>;">
			  <td  align="center" class='ag_name' ><?php echo $rows['ab_username']?></td>
