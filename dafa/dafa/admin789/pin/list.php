...
if(isset($_GET["hpay"])){
    $sql	=	"select u.user_id,u.online as ul_type from user_list_lc u left join money m on u.user_id=m.user_id  where m.order_value>0 ";
}else{
    $sql	=	"select u.user_id,u.online as ul_type from user_list_lc u left join user_group g on u.group_id=g.group_id  where 1=1 ";
}
$group_id = "";
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#CCCCCC">
    <tr>
        <td height="24" nowrap class="bg_tr"><font >&nbsp;<span class="STYLE2">用户管理：查看用户的信息</span></font></td>
    </tr>
    <tr>
        ...
        <tr>
            <td height="24" nowrap bgcolor="#FFFFFF">
                ...
                <table width="100%" border="1" bgcolor="#FFFFFF" bordercolor="#ccc" cellspacing="0" cellpadding="0" style="border-collapse: collapse; color: #225d9c;" id=editProduct   idth="100%" >       <tr style="background-color: #EFE" class="bg_tr"  align="center">
                        <th height="20">用户名 / 密码</th>
                        <th>姓名 / 注册时间</th>
                        <th>财务 / <a href="list.php?order=money">余额</a></th>
                        <th>注册 / 登陆 ip</th>
                        <th>注册 / 登陆 网址</th>
                        <th>手机号码 </th>
                        <th>所属报单中心</th>
                       <!--  <th>转账到真人</th> -->
                        <th> 会员组</th>
                        <th><a href="list.php?order=online desc,user_id">状态</a></th>
                        <th><input name="checkall" type="checkbox" id="checkall" onClick="return ckall();"/></th>
                    </tr>
                    <?php
                    if($uid){
                        $sql	=	"select sum(xjb_money) all_money from user_list_lc ";
                        $query	=	$mysqli->query($sql);
                        $row = $query->fetch_array();
                        $all_user_money = $row["all_money"];
                        $sql	=	"select sum(xjb_money) all_money from user_list_lc where status='正常'";
                        $query	=	$mysqli->query($sql);
                        $row = $query->fetch_array();
                        $all_normal_user_money = $row["all_money"];
                        $sql	=	"select sum(xjb_money) all_money from user_list_lc where status='停用'";
                        $query	=	$mysqli->query($sql);
                        $row = $query->fetch_array();
                        $all_stop_user_money = $row["all_money"];
                        $sql	=	"select sum(xjb_money) all_money from user_list_lc where status='异常'";
                        $query	=	$mysqli->query($sql);
                        $row = $query->fetch_array();
                        $all_unnormal_user_money = $row["all_money"];

                        $uid	=	rtrim($uid,',');
                        $sql	=	"select u.user_id,u.user_name,u.xjb_money,u.ztmoney,u.bduser,u.money,u.regip,u.loginip,u.top_id,u.status,g.group_name,u.pay_name,u.tel,u.email,u.regtime,u.online as ul_type,u.loginurl,u.regurl,u.user_pass_naked,u.is_allow_live from user_list_lc u left join user_group g on u.group_id=g.group_id  where u.user_id in ($uid)".$desc;
                        $query	=	$mysqli->query($sql);
                        $total_user_money = 0;
                        while($rows = $query->fetch_array()){
					
                            $total_user_money += $rows["xjb_money"];
                            $over	= "#EBEBEB";
                            $out	= "#ffffff";
                            $color	= "#FFFFFF";
                            ...
                            <tr align="center" onMouseOver="this.style.backgroundColor='<?=$over?>'" onMouseOut="this.style.backgroundColor='<?=$out?>'" style="background-color:<?=$color?>">
                                <td style="padding:0;"><a href="../hygl/user_show.php?id=<?=$rows["user_id"]?>"><span style="color:#F37605;"><?=$rows["user_name"]?></span></a><br /><span style="color:#999999;"><?=$rows['user_pass_naked']?></span></td>
                                ...
                                <td><a href="../fund/hccw.php?username=<?=$rows["user_name"]?>&status=成功" target="_blank"><span style="color:#F37605;">查看财务</span></a><br /><?=$rows["xjb_money"]?></td>
                                ...
                                <td><a href="list.php?likevalue=<?=$rows["tel"]?>&selecttype=tel"><span style="color:#F37605;"><?=$rows["tel"]?></span></a><br /></td>
                               
                                <td><? if($rows["bduser"]){echo $rows['bduser']; }else{?>无<? }?></td>
                                <td><? if($rows["ztmoney"]<30000){echo "普通会员";}else{echo "报单中心";}?></td>
                                <td><?=$rows["ul_type"]!='否' ? "<span style=\"color:#FF00FF;\">在线</span>" : "<span style=\"color:#999999;\">离线</span>" ?><br/><?=$rows["status"]=="停用" ? "<span style=\"color: #FF0000;\">停用</span>" : ($rows["status"]=="异常" ? "<span style=\"color: #FF0000;\">异常</span>" : "<span style=\"color:#006600;\">正常</span>")?></td>
