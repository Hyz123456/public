...
if($lottery_type=="重庆时时彩"){
    $gType = "cq";
}elseif($lottery_type=="江西时时彩"){
    $gType = "jx";
}elseif($lottery_type=="天津时时彩"){
    $gType = "tj";
}
...
if($_GET["action"]=="add" && $id==0){
    ...
    if($row && $row["id"]){
        message("该期彩票结果已存在，请查询后编辑。","result_b5.php?type=$lottery_type&s_time=$query_time");
    }else{
        $sql		=	"insert into lottery_result_".$gType."(qishu,create_time,datetime,ball_1,ball_2,ball_3,ball_4,ball_5) values (".$qishu.",'".$create_time."','".$datetime."',".$ball_1.",".$ball_2.",".$ball_3.",".$ball_4.",".$ball_5.")";
        $mysqli->query($sql);
    }
    ...
    $ball_4		=	$_POST["ball_4"];
    $ball_5		=	$_POST["ball_5"];
    $sql		=	"update lottery_result_".$gType." set prev_text='".$prev_text."', qishu=".$qishu.",datetime='".$datetime."',ball_1=".$ball_1.",ball_2=".$ball_2.",ball_3=".$ball_3.",ball_4=".$ball_4.",ball_5=".$ball_5." where id=".$id."";
    $mysqli->query($sql);
}
elseif($_GET["action"]=="delete" && $id>0){
    ...
    $query	=	$mysqli->query($sql);
    //$row    =	$query->fetch_array();
    if($query){echo '<script>alert("删除成功"); window.href.location="/admin/Lottery/result/B5/result_b5.php?status=0&type=%E9%87%8D%E5%BA%86%E6%97%B6%E6%97%B6%E5%BD%A9";</script>';}
	else{echo '<script>alert("删除失败"); window.href.location="/admin/Lottery/result/B5/result_b5.php?status=0&type=%E9%87%8D%E5%BA%86%E6%97%B6%E6%97%B6%E5%BD%A9";</script>';}
}
...
<head>
    ...
    <script language="javascript" src="query_b5.js"></script>
...
<tr>
    ...
    <td  align="left" bgcolor="#5a5a5a" style="color:#FFF"><strong><?=$lottery_type?></strong></td>
</tr>
<tr>
...
<table width="100%" border="0" cellpadding="5" cellspacing="1" class="font12" style="margin-top:2px;" bgcolor="#ccc">
    ...
    $sql = "SELECT id,qishu,create_time,datetime,state,prev_text,
        ball_1,ball_2,ball_3,ball_4,ball_5
        FROM lottery_result_".$gType." WHERE DATE_FORMAT(datetime,'%Y-%m-%d') = '$query_time'";
    if($qishu_query){
    ...
    while($rows = $query->fetch_array()){
        ...
        if($rows['state']=="0"){
            $ok = '<a href="js_b5.php?qi='.$rows['qishu'].'&jsType=0&type='.$lottery_type.'&gtype='.$gType.'&s_time='.$query_time.'" title="点击结算"><font color="#0000FF">未结算</font></a>';
        }else{
            $ok = '<a href="js_b5.php?qi='.$rows['qishu'].'&jsType=1&type='.$lottery_type.'&gtype='.$gType.'&s_time='.$query_time.'" title="重新结算"><font color="#FF0000">已结算</font></a>';
        }
        if($rows['state']=="2"){
        ...
        <tr align="center" onMouseOver="this.style.backgroundColor='<?=$over?>'" onMouseOut="this.style.backgroundColor='<?=$out?>'" style="background-color:<?=$color?>; line-height:20px;">
            ...
            <td align="center" valign="middle"><img src="/images/Lottery/Images/Ball_2/<?=$rows['ball_4']?>.png"></td>
            <td align="center" valign="middle"><img src="/images/Lottery/Images/Ball_2/<?=$rows['ball_5']?>.png"></td>
