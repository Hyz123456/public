...
if($_GET["action"]=="add" && $id==0){
    ...
    if($row && $row["id"]){
        message("该期彩票结果已存在，请查询后编辑。","result_gdsf.php?s_time=$query_time");
    }else{
        $sql		=	"insert into lottery_result_gdsf(qishu,create_time,datetime,ball_1,ball_2,ball_3,ball_4,ball_5,ball_6,ball_7,ball_8) values (".$qishu.",'".$create_time."','".$datetime."',".$ball_1.",".$ball_2.",".$ball_3.",".$ball_4.",".$ball_5.",".$ball_6.",".$ball_7.",".$ball_8.")";
        $mysqli->query($sql);
    }
}elseif($_GET["action"]=="edit" && $id>0){
    $sql		=	"select * from lottery_result_gdsf WHERE id='$id'";
    $query	=	$mysqli->query($sql);
    $row    =	$query->fetch_array();
    ...
    $ball_7		=	$_POST["ball_7"];
    $ball_8		=	$_POST["ball_8"];
    $sql		=	"update lottery_result_gdsf set prev_text='".$prev_text."', qishu=".$qishu.",datetime='".$datetime."',ball_1=".$ball_1.",ball_2=".$ball_2.",ball_3=".$ball_3.",ball_4=".$ball_4.",ball_5=".$ball_5.",ball_6=".$ball_6.",ball_7=".$ball_7.",ball_8=".$ball_8." where id=".$id."";
    $mysqli->query($sql);
}
...
<head>
    ...
    <script language="javascript" src="query_gdsf.js"></script>
...
if($id>0 && !isset($_GET['action'])){
    $sql = "SELECT id,qishu,create_time,datetime,state,prev_text,
            ...
            FROM lottery_result_gdsf WHERE id=$id limit 0,1";
    $query	=	$mysqli->query($sql);
...
<tr>
    ...
    <td  align="left" bgcolor="#5a5a5a" style="color:#FFF"><strong>广东十分彩</strong></td>
</tr>
<tr>
...
<table width="100%" border="0" cellpadding="5" cellspacing="1" class="font12" style="margin-top:2px;" bgcolor="#ccc">
    ...
    $sql = "SELECT id,qishu,create_time,datetime,state,prev_text,
        ball_1,ball_2,ball_3,ball_4,ball_5,ball_6,ball_7,ball_8
        FROM lottery_result_gdsf WHERE DATE_FORMAT(datetime,'%Y-%m-%d') = '$query_time'";
    if($qishu_query){
    ...
    while($rows = $query->fetch_array()){
        ...
        if($rows['state']=="0"){
            $ok = '<a href="js_gdsf.php?qi='.$rows['qishu'].'&jsType=0&s_time='.$query_time.'" title="点击结算"><font color="#0000FF">未结算</font></a>';
        }else{
            $ok = '<a href="js_gdsf.php?qi='.$rows['qishu'].'&jsType=1&s_time='.$query_time.'" title="重新结算"><font color="#FF0000">已结算</font></a>';
        }
        if($rows['state']=="2"){
