...
if($_GET["action"]=="add" && $id==0){
    ...
    if($row && $row["id"]){
        message("该期彩票结果已存在，请查询后编辑。","result_bjpk.php?s_time=$query_time");
    }else{
        $sql		=	"insert into lottery_result_bjpk(qishu,create_time,datetime,ball_1,ball_2,ball_3,ball_4,ball_5,ball_6,ball_7,ball_8,ball_9,ball_10) values (".$qishu.",'".$create_time."','".$datetime."',".$ball_1.",".$ball_2.",".$ball_3.",".$ball_4.",".$ball_5.",".$ball_6.",".$ball_7.",".$ball_8.",".$ball_9.",".$ball_10.")";
        $mysqli->query($sql);
    }
}elseif($_GET["action"]=="edit" && $id>0){
    $sql		=	"select * from lottery_result_bjpk WHERE id='$id'";
    $query	=	$mysqli->query($sql);
    $row    =	$query->fetch_array();
    $prev_text = "修改时间：".(date("Y-m-d H:i:s",time()))."。\n修改前内容：".$row["ball_1"].",".$row["ball_2"].",".$row["ball_3"].",".$row["ball_4"].",".$row["ball_5"].",".$row["ball_6"].",".$row["ball_7"].",".$row["ball_8"].",".$row["ball_9"].",".$row["ball_10"]."。\n修改后内容：".$_POST["ball_1"].",".$_POST["ball_2"].",".$_POST["ball_3"].",".$_POST["ball_4"].",".$_POST["ball_5"].",".$_POST["ball_6"].",".$_POST["ball_7"].",".$_POST["ball_8"].",".$_POST["ball_9"].",".$_POST["ball_10"]."。".'\n\n'.$row["prev_text"];
    ...
    $ball_9		=	$_POST["ball_9"];
    $ball_10	=	$_POST["ball_10"];
    $sql		=	"update lottery_result_bjpk set prev_text='".$prev_text."', qishu=".$qishu.",datetime='".$datetime."',ball_1=".$ball_1.",ball_2=".$ball_2.",ball_3=".$ball_3.",ball_4=".$ball_4.",ball_5=".$ball_5.",ball_6=".$ball_6.",ball_7=".$ball_7.",ball_8=".$ball_8.",ball_9=".$ball_9.",ball_10=".$ball_10." where id=".$id."";
    $mysqli->query($sql);
}elseif($_GET["action"]=="delete" && $id>0){
    $sql		=	"delete from lottery_result_bjpk WHERE id='$id'";
    $query	=	$mysqli->query($sql);
	if($query){echo '<script>alert("删除成功"); window.href.location="/admin/Lottery/result/BJPK/result_bjpk.php?status=0&type=%E5%8C%97%E4%BA%ACPK%E6%8B%BE";</script>';}
	else{echo '<script>alert("删除失败"); window.href.location="/admin/Lottery/result/BJPK/result_bjpk.php?status=0&type=%E5%8C%97%E4%BA%ACPK%E6%8B%BE";</script>';}
...
<head>
    ...
    <script language="javascript" src="query_bjpk.js"></script>
...
if($id>0 && !isset($_GET['action'])){
    $sql = "SELECT id,qishu,create_time,datetime,state,prev_text,
            ...
            FROM lottery_result_bjpk WHERE id=$id limit 0,1";
    $query	=	$mysqli->query($sql);
...
<tr>
    <td  align="left" bgcolor="#5a5a5a" style="color:#FFF">彩票类别：</td>
    <td  align="left" bgcolor="#5a5a5a" style="color:#FFF"><strong>北京PK拾</strong></td>
</tr>
<tr>
...
<table width="100%" border="0" cellpadding="5" cellspacing="1" class="font12" style="margin-top:2px;" bgcolor="#ccc">
    ...
    while($rows = $query->fetch_array()){
        ...
        if($rows['state']=="0"){
            $ok = '<a href="js_bjpk.php?qi='.$rows['qishu'].'&jsType=0&s_time='.$query_time.'" title="点击结算"><font color="#0000FF">未结算</font></a>';
        }else{
            $ok = '<a href="js_bjpk.php?qi='.$rows['qishu'].'&jsType=1&s_time='.$query_time.'" title="重新结算"><font color="#FF0000">已结算</font></a>';
        }
        if($rows['state']=="2"){
        ...
        <tr align="center" onMouseOver="this.style.backgroundColor='<?=$over?>'" onMouseOut="this.style.backgroundColor='<?=$out?>'" style="background-color:<?=$color?>; line-height:20px;">
            <td height="25" align="center" valign="middle" style="padding:0;">北京PK拾</td>
            <td align="center" valign="middle" style="padding:0;"><?=$rows['qishu']?></td>
            <td align="center" valign="middle" style="padding:0;"><?=$rows['datetime']?></td>
            <td align="center" valign="middle" style="padding:0;"><img src="/images/Lottery/Images/Ball_3/<?=$rows['ball_1']?>.png"></td>
            <td align="center" valign="middle" style="padding:0;"><img src="/images/Lottery/Images/Ball_3/<?=$rows['ball_2']?>.png"></td>
            <td align="center" valign="middle" style="padding:0;"><img src="/images/Lottery/Images/Ball_3/<?=$rows['ball_3']?>.png"></td>
            <td align="center" valign="middle" style="padding:0;"><img src="/images/Lottery/Images/Ball_3/<?=$rows['ball_4']?>.png"></td>
            <td align="center" valign="middle" style="padding:0;"><img src="/images/Lottery/Images/Ball_3/<?=$rows['ball_5']?>.png"></td>
            <td align="center" valign="middle" style="padding:0;"><img src="/images/Lottery/Images/Ball_3/<?=$rows['ball_6']?>.png"></td>
            <td align="center" valign="middle" style="padding:0;"><img src="/images/Lottery/Images/Ball_3/<?=$rows['ball_7']?>.png"></td>
            <td align="center" valign="middle" style="padding:0;"><img src="/images/Lottery/Images/Ball_3/<?=$rows['ball_8']?>.png"></td>
            <td align="center" valign="middle" style="padding:0;"><img src="/images/Lottery/Images/Ball_3/<?=$rows['ball_9']?>.png"></td>
            <td align="center" valign="middle" style="padding:0;"><img src="/images/Lottery/Images/Ball_3/<?=$rows['ball_10']?>.png"></td>
            <td align="center" valign="middle" style="padding:0;"><?=$ok?></td>
            <td align="center" valign="middle" style="padding:0;"><?=$again?></td>
