...
if($_GET["action"]=="add" && $id==0){
    ...
    if($row && $row["id"]){
        message("该期彩票结果已存在，请查询后编辑。","result_gd11.php?s_time=$query_time");
    }else{
        $sql		=	"insert into lottery_result_gd11(qishu,create_time,datetime,ball_1,ball_2,ball_3,ball_4,ball_5) values (".$qishu.",'".$create_time."','".$datetime."',".$ball_1.",".$ball_2.",".$ball_3.",".$ball_4.",".$ball_5.")";
        $mysqli->query($sql);
    }
}elseif($_GET["action"]=="edit" && $id>0){
    $sql		=	"select * from lottery_result_gd11 WHERE id='$id'";
    $query	=	$mysqli->query($sql);
    $row    =	$query->fetch_array();
    ...
    $ball_4		=	$_POST["ball_4"];
    $ball_5		=	$_POST["ball_5"];
    $sql		=	"update lottery_result_gd11 set prev_text='".$prev_text."', qishu=".$qishu.",datetime='".$datetime."',ball_1=".$ball_1.",ball_2=".$ball_2.",ball_3=".$ball_3.",ball_4=".$ball_4.",ball_5=".$ball_5." where id=".$id."";
    $mysqli->query($sql);
}
...
<head>
    ...
    <script language="javascript" src="query_gd11.js"></script>
...
if($id>0 && !isset($_GET['action'])){
    $sql = "SELECT id,qishu,create_time,datetime,state,prev_text,
            ...
            FROM lottery_result_gd11 WHERE id=$id limit 0,1";
    $query	=	$mysqli->query($sql);
...
<tr>
    ...
    <td  align="left" bgcolor="#5a5a5a" style="color:#FFF"><strong>广东十一选五</strong></td>
</tr>
<tr>
...
<table width="100%" border="0" cellpadding="5" cellspacing="1" class="font12" style="margin-top:2px;" bgcolor="#ccc">
    ...
    while($rows = $query->fetch_array()){
        ...
        if($rows['state']=="0"){
            $ok = '<a href="js_gd11.php?qi='.$rows['qishu'].'&jsType=0&s_time='.$query_time.'" title="点击结算"><font color="#0000FF">未结算</font></a>';
        }else{
            $ok = '<a href="js_gd11.php?qi='.$rows['qishu'].'&jsType=1&s_time='.$query_time.'" title="重新结算"><font color="#FF0000">已结算</font></a>';
        }
        if($rows['state']=="2"){
