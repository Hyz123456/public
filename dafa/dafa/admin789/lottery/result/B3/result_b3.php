...
if($_GET["action"]=="add" && $id==0){
    ...
    $query	=	$mysqli->query($sql);
    if($query){echo '<script>alert("删除成功"); window.href.location="/admin/Lottery/result/B3/result_b3.php?status=0&type=3D%E5%BD%A9";</script>';}
	else{echo '<script>alert("删除失败"); window.href.location="/admin/Lottery/result/B3/result_b3.php?status=0&type=3D%E5%BD%A9";</script>';}
}
