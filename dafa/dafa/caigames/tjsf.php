...
<html>
	...
	<body>
		...
		<aside>
			...
			<?php 
				...
				$sql='select max(qishu) as qishu from lottery_result_tjsf';
				$query=$mysqli->query($sql) or die('error!');
				$arr=$query->fetch_array();

				$mq=$arr['qishu'];
				$sql='select * from lottery_result_tjsf where qishu="'.$mq.'"';
				$query=$mysqli->query($sql) or die('error!');
			...
			<div  class="lottery_kind">
				<a href="javascript:;" class="active">天津快乐十分</a>
	...
	<section class="lotterListBox">
		...
		<form action="../member/lottery/order/order_tjsf.php?style=wap" method="post" id="fomes"  class='box'>
			...
			<div class="numBox">
				...
				<?php
						...
						$sql="select * from odds_lottery where sub_type='正码和特别号' and lottery_type='天津十分彩' and ball_type='ball_".$j."' ";
						$query=$mysqli->query($sql) or die('error!');
						$arr=$query->fetch_array();

						$sql="select * from odds_lottery where sub_type='方位中发白' and lottery_type='天津十分彩' and ball_type='ball_".$j."' ";
						$query=$mysqli->query($sql) or die('error!');
				...
				<p>
					...
					<span><?=$arr['h'.$i]?></span>
					<input name="<?='ball_'.$j.'_'.$i?>" type="text" /></dis></dis>
			...
			<div class="numBox">
				...
				<?php
					$sql="select * from odds_lottery where sub_type='总和龙虎' and lottery_type='天津十分彩' ";
					$query=$mysqli->query($sql) or die('error!');
					$arr=$query->fetch_array();
				?>
...
<div class="popover-backdrop">
    <div class="popover-wrapper">
        <div class="list"> 
            <a href="/member/final1/LT_result.php?gtype=TJSF">开奖历史</a>
        </div>
		...
		<script>
			...
			(function(){
					function updateOpenTime(){
						$.post("../member/lottery/class/odds_tjsf.php", function(data){
