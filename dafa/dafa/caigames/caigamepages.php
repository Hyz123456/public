	...
	<body>
		...
		<aside>
			...
			<?php 
				...
				$sql='select max(qishu) as qishu from lottery_result_cq';
				$query=$mysqli->query($sql) or die('error!');
				$arr=$query->fetch_array();

				$mq=$arr['qishu'];
				$sql='select * from lottery_result_cq where qishu="'.$mq.'"';
				$query=$mysqli->query($sql) or die('error!');
			...
			<div  class="lottery_kind">
				<a href="javascript:;" class="active">重庆时时彩</a>
				<a href="/main.php?2" id='back' style='margin-left:1.3rem;' >back</a>
			</div>
		</aside>
	<section class="lotterListBox">
		<div class="lotteryBtn">
			<a class="active" href="javascript:;">珠仔<span class="smallround"></span></a>
			<a href="javascript:;">总和龙虎<span class="smallround"></span></a>
			<a href="javascript:;">组合<span class="smallround"></span></a>
			<a href="javascript:;">斗牛<span class="smallround"></span></a>
		</div>
		<form action="../member/lottery/order/order_cq_ssc.php?style=wap" method="post" id="fomes"  class='box'>
			...
			<div class="numBox">
				...
				<?php
					for($j=1;$j<6;$j++){
						$sql="select * from odds_lottery_normal where lottery_type='重庆时时彩' and ball_type='ball_".$j."' ";
						$query=$mysqli->query($sql) or die('error!');
						$arr=$query->fetch_array();

						$sql="select * from odds_lottery_normal where sub_type='两面' and lottery_type='重庆时时彩' and ball_type='part1' ";
						$query=$mysqli->query($sql) or die('error!');
						$arr1=$query->fetch_array();
				?>
				...
				<p>
                                        <dis>
					<span class="bg_blue"><?=$i-1?></span>
					<span><?=$arr['h'.($i-1)]?></span>
					<input name="<?='ball_'.$j.'_'.$i?>" id="<?='ball_'.$j.'_'.$i?>" type="text" />
                                        </dis><dis>
					<span class="bg_green"><?=$i?></span>
					<span><?=$arr['h'.($i)]?></span>
					<input name="<?='ball_'.$j.'_'.($i+1)?>" id="<?='ball_'.$j.'_'.($i+1)?>" type="text" />
                                        </dis>
				</p>
				<?php
					}
				...
				<p>     <dis>
					<span>大</span>
					<span><?=$arr1['h0']?></span>
					<input name="<?='ball_'.$j.'_11'?>"  id="<?='ball_'.$j.'_11'?>" type="text" />
                                        </dis><dis>
					<span>小</span>
					<span><?=$arr1['h1']?></span>
					<input name="<?='ball_'.$j.'_12'?>" id="<?='ball_'.$j.'_12'?>" type="text" />
                                        </dis>
				</p>
				<p>
                                        <dis>
					<span>单</span>
					<span><?=$arr1['h2']?></span>
					<input name="<?='ball_'.$j.'_13'?>" id="<?='ball_'.$j.'_13'?>" type="text" />
                                        </dis><dis>
					<span>双</span>
					<span><?=$arr1['h3']?></span>
					<input name="<?='ball_'.$j.'_14'?>" id="<?='ball_'.$j.'_14'?>" type="text" />
                                        </dis>
				</p>
			...
			<div class="numBox">
				...
				<p>
					...
					<span>总和单</span>
					<span><?=$arr['h2']?></span>
					<input name="<?='ball_6_3'?>" id="<?='ball_6_3'?>" type="text" />
                                        </dis><dis>
					<span>总和双</span>
					<span><?=$arr['h3']?></span>
				...
				<p>
					...
					<span>虎</span>
					<span><?=$arr['h5']?></span>
					<input name="<?='ball_6_6'?>" id="<?='ball_6_6'?>" type="text" />
                                        </dis>
				</p>
				<p>
                                        <dis>
					<span>和</span>
					<span><?=$arr['h6']?></span>
					<input name="<?='ball_6_7'?>"  id="<?='ball_6_7'?>" type="text" />
                                        </dis>
				</p>
			</div>
			...
			<div class="numBox">
				...
				<p>
					...
					<span>豹子</span>
					<span><?=$arr['h0']?></span>
					<input name="<?='ball_'.$i.'_1'?>" id="<?='ball_'.$i.'_1'?>" type="text" />
                                        </dis><dis>
					<span>顺子</span>
					<span><?=$arr['h1']?></span>
					<input name="<?='ball_'.$i.'_2'?>" id="<?='ball_'.$i.'_2'?>" type="text" />
                                        </dis>
				</p>
				<p> 
                                        <dis>
					<span>对子</span>
					<span><?=$arr['h2']?></span>
					<input name="<?='ball_'.$i.'_3'?>" id="<?='ball_'.$i.'_3'?>" type="text" />
                                        </dis><dis>
					<span>半顺</span>
					<span><?=$arr['h3']?></span>
					<input name="<?='ball_'.$i.'_4'?>" id="<?='ball_'.$i.'_4'?>" type="text" />
                                        </dis>
				</p>
				<p>
                                        <dis>
					<span>杂六</span>
					<span><?=$arr['h4']?></span>
					<input name="<?='ball_'.$i.'_5'?>" id="<?='ball_'.$i.'_5'?>" type="text" />
                                        </dis>
				</p>
				<?php
					}
			...
			<div class="numBox">
				...
				<?php
					$sql="select * from odds_lottery_normal where sub_type='牛牛' and lottery_type='重庆时时彩' ";
					$query=$mysqli->query($sql) or die('error!');
					$arr=$query->fetch_array();
				?>
				...
				<p>
					...
					<span>无牛</span>
					<span><?=$arr['h0']?></span>
					<input name="<?='ball_10_1'?>" id="<?='ball_10_1'?>" type="text" />
                                        </dis><dis>
					<span>牛1</span>
					<span><?=$arr['h1']?></span>
					<input name="<?='ball_10_2'?>" id="<?='ball_10_2'?>"  type="text" />
                                        </dis>
				</p>
				<p>
                                        <dis>
					<span>牛2</span>
					<span><?=$arr['h2']?></span>
					<input name="<?='ball_10_3'?>" id="<?='ball_10_3'?>" type="text" />
                                        </dis><dis>
					<span>牛3</span>
					<span><?=$arr['h3']?></span>
					<input name="<?='ball_10_4'?>" id="<?='ball_10_4'?>" type="text" />
                                        </dis>
				</p>
				<p>
                                        <dis>
					<span>牛4</span>
					<span><?=$arr['h4']?></span>
					<input name="<?='ball_10_5'?>" id="<?='ball_10_5'?>" type="text" />
                                        </dis><dis>
					<span>牛5</span>
					<span><?=$arr['h5']?></span>
					<input name="<?='ball_10_6'?>" id="<?='ball_10_6'?>" type="text" />
                                        </dis>
				</p>
				<p>
                                        <dis>
					<span>牛6</span>
					<span><?=$arr['h6']?></span>
					<input name="<?='ball_10_7'?>" id="<?='ball_10_7'?>" type="text" />
                                        </dis><dis>
					<span>牛7</span>
					<span><?=$arr['h7']?></span>
					<input name="<?='ball_10_8'?>" id="<?='ball_10_8'?>" type="text" />
                                        <dis/>
				</p>
				<p>
					<dis><span>牛8</span>
					<span><?=$arr['h8']?></span>
					<input name="<?='ball_10_9'?>" id="<?='ball_10_9'?>" type="text" /><dis/>
					<dis><span>牛9</span>
					<span><?=$arr['h9']?></span>
					<input name="<?='ball_10_10'?>" id="<?='ball_10_10'?>" type="text" /><dis/>
				</p>
				<p>
					<dis><span>牛牛</span>
					<span><?=$arr['h10']?></span>
					<input name="<?='ball_10_11'?>" id="<?='ball_10_11'?>" type="text" /><dis/>
					<dis><span>牛大</span>
					<span><?=$arr['h11']?></span>
					<input name="<?='ball_10_12'?>" id="<?='ball_10_12'?>" type="text" /><dis/>
				</p>
				<p>
					<dis><span>牛小</span>
					<span><?=$arr['h12']?></span>
					<input name="<?='ball_10_13'?>" id="<?='ball_10_13'?>" type="text" /><dis/>
					<dis><span>牛单</span>
					<span><?=$arr['13']?></span>
					<input name="<?='ball_10_14'?>" id="<?='ball_10_14'?>" type="text" /><dis/>
				</p>
				<p>
					<dis><span>牛双</span>
					<span><?=$arr['h14']?></span>
					<input name="<?='ball_10_15'?>" id="<?='ball_10_15'?>" type="text" /><dis/>
				</p>
			</div>
				...
				<div class="sub">
                                ...
                                <div class="popover-backdrop">
                                    <div class="popover-wrapper">
                                        <div class="list"> 
                                            <a href="../member/final1/LT_result.php?gtype=CQ" >开奖历史</a>
                                        </div>
