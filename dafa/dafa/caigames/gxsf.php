...
<html>
	...
	<body>
		...
		<aside>
			...
			<?php 
				...
				$sql='select max(qishu) as qishu from lottery_result_gxsf';
				$query=$mysqli->query($sql) or die('error!');
				$arr=$query->fetch_array();

				$mq=$arr['qishu'];
				$sql='select * from lottery_result_gxsf where qishu="'.$mq.'"';
				$query=$mysqli->query($sql) or die('error!');
			...
			<div  class="lottery_kind">
				<a href="javascript:;" class="active">广西十分彩</a>
	...
	<section class="lotterListBox">
		...
		<form action="../member/lottery/order/order_gxsf.php?style=wap" method="post" id="fomes"  class='box'>
			<?php for($j=1;$j<6;$j++){?>
			<div class="numBox">
				<nav>
				...
				<?php
						$sql="select * from odds_lottery where sub_type='正码和特别号' and lottery_type='广西十分彩' and ball_type='ball_".$j."' ";
						$query=$mysqli->query($sql) or die('error!');
						$arr=$query->fetch_array();
				?>
				...
				?>
				<p>
					<dis><span class="bg_blue">21</span>
					<span><?=$arr['h21']?></span>
					<input name="<?='ball_'.$j.'_21'?>" type="text" /></dis>
				</p>
				<p>
					<dis><span>大</span>
					<span><?=$arr['h29']?></span>
					<input name="<?='ball_'.$j.'_22'?>" type="text" /></dis>
					<dis><span>小</span>
					<span><?=$arr['h30']?></span>
					<input name="<?='ball_'.$j.'_23'?>" type="text" /></dis>
				</p>
				<p>
					<dis><span>单</span>
					<span><?=$arr['h31']?></span>
					<input name="<?='ball_'.$j.'_24'?>" type="text" /></dis>
					<dis><span>双</span>
					<span><?=$arr['h32']?></span>
					<input name="<?='ball_'.$j.'_25'?>" type="text" /></dis>
				</p>
			...
			<div class="numBox">
				...
				<?php
					$sql="select * from odds_lottery where sub_type='总和龙虎和' and lottery_type='广西十分彩' ";
					$query=$mysqli->query($sql) or die('error!');
					$arr=$query->fetch_array();
				?>
			...
			<div class="numBox">
				...
				<?php
					$sql="select * from odds_lottery where sub_type='顺子杂六' and lottery_type='广东十一选五' ";
					$query=$mysqli->query($sql) or die('error!');
					$arr=$query->fetch_array();
					for($i=7;$i<10;$i++){
				?>
...
<div class="popover-backdrop">
    <div class="popover-wrapper">
        <div class="list"> 
            <a href="/member/final1/LT_result.php?gtype=GXSF">开奖历史</a>
        </div>
		...
		<script>
			...
			(function(){
					function updateOpenTime(){
						$.post("../member/lottery/class/odds_gxsf.php", function(data){
