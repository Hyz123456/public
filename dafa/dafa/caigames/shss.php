...
<html>
	...
	<body>
		...
		<aside>
			...
			<?php 
				...
				$sql='select max(qishu) as qishu from lottery_result_t3';
				$query=$mysqli->query($sql) or die('error!');
				$arr=$query->fetch_array();

				$mq=$arr['qishu'];
				$sql='select * from lottery_result_t3 where qishu="'.$mq.'"';
				$query=$mysqli->query($sql) or die('error!');
			...
			<div  class="lottery_kind">
				<a href="javascript:;" class="active">上海时时乐</a>
	...
	<section class="lotterListBox">
		...
		<form action="../member/lottery/order/order_shssl.php?style=wap" method="post" id="fomes"  class='box'>
			...
			<div class="numBox">
				...
				<?php
					$sql="select * from odds_lottery_normal where lottery_type='上海时时乐' and sub_type='佰定位'";
					$query=$mysqli->query($sql) or die('error!');
					$arr=$query->fetch_array();
					for($i=0;$i<10;$i+=2){
				?>
				...
				<?php
					...
					$sql="select * from odds_lottery_normal where lottery_type='上海时时乐' and sub_type='两面' ";
					$query=$mysqli->query($sql) or die('error!');
					$arr1=$query->fetch_array();
				?>
				<p>
				...
				<?php
					$sql="select * from odds_lottery_normal where lottery_type='上海时时乐' and sub_type='拾定位'";
					$query=$mysqli->query($sql) or die('error!');
					$arr=$query->fetch_array();
					for($i=0;$i<10;$i+=2){
				?>
				...
				<?php
					$sql="select * from odds_lottery_normal where lottery_type='上海时时乐' and sub_type='个定位'";
					$query=$mysqli->query($sql) or die('error!');
					$arr=$query->fetch_array();
					for($i=0;$i<10;$i+=2){
				?>
			...
			<div class="numBox">
				...
				<?php
					$sql="select * from odds_lottery_normal where sub_type='总和龙虎和' and lottery_type='上海时时乐' ";
					$query=$mysqli->query($sql) or die('error!');
					$arr=$query->fetch_array();
				?>
			...
			<div class="numBox">
				...
				<?php
					$sql="select * from odds_lottery_normal where sub_type='3连' and lottery_type='上海时时乐' ";
					$query=$mysqli->query($sql) or die('error!');
					$arr=$query->fetch_array();
				?>
				<p>
			...
			<div class="numBox">
				...
				<?php
					$sql="select * from odds_lottery_normal where sub_type='跨度' and lottery_type='上海时时乐' ";
					$query=$mysqli->query($sql) or die('error!');
					$arr=$query->fetch_array();
					for($i=0;$i<10;$i+=2){
				?>
...
<div class="popover-backdrop">
    <div class="popover-wrapper">
        <div class="list"> 
            <a href="/member/final1/LT_result.php?gtype=T3">开奖历史</a>
        </div>
		...
		<script>
			...
			(function(){
					function updateOpenTime(){
						$.post("../member/lottery/class/odds_shssl.php", function(data){
