...
function GetSlotGame(gametype, currentPage, type, hot) {
    var pageCount = currentPage;
    currentPage = currentPage * totalRecord;

    jQuery.ajax({
        cache: false,
        type: "POST",
        url: "sv.asmx/GetSlotGame",
        data: { "gameType": gametype, "start": currentPage, "numRow": totalRecord, "sort": "", "type": type, "hot": hot, "html5": html5 },
        async: true,
        success: function (data) {
            var m = jQuery.parseJSON(data);
            jQuery(".lst-slots-game").html("");
            ...
            paging(pageCount, gametype, type, hot);
        },
        error: function (xhr, ajaxOptions, thrownError) {
        }
    });
...
function paging(currentPage, gameType, type, hot) {
    ...
    jQuery.ajax({
        cache: false,
        type: "POST",
        url: "sv.asmx/GetTotalGameByGameType",
        data: { "gameType": gameType, "type": type, "hot": hot, "html5": html5 },
        success: function (data) {
            totalRows = data;     
            jQuery("#control-paging").pagination('updateItems', totalRows);
        },
        error: function (xhr, ajaxOptions, thrownError) {
        }
    });
