...
include_once($C_Patch."/app/member/class/lottery_result_gdsf.php");
...
include_once("./Js_Class.php");
    ...
    //获取开奖号码
    $rs = lottery_result_gdsf::getLotteryResult($qi);
    $hm 		= array();
    $hm[]		= $rs['ball_1'];
    $hm[]		= $rs['ball_2'];
    $hm[]		= $rs['ball_3'];
    ...
    $jiesuanString = "结算成功。";
    $url = "result_gdsf.php?s_time=$query_time";
    $stateType = "1";

    if($jsType==1){//状态为已结算，对所有的订单进行结算，需要从客户那边收回钱然后再进行结算
        //获取已结算的订单
        $orders = lottery_order::getOrdersJs("GDSF",$qi);
            ...
            foreach($orders as $order){
                ...
                if($order['is_win']=="1" || $order['is_win']=="2" || ($order['is_win']=="0" && $order['fs']>0)){
                    ...
                    $sql = "INSERT INTO `money_log` (`user_id`,`order_num`,`about`,`update_time`,`type`,`order_value`,`assets`,`balance`)
                            VALUES ('$userid','$datereg','广东十分彩',now(),'彩票重新结算-扣钱','$bet_money_total','$assets','$balance');";
    ...
    //获取未结算的订单
    $orders = lottery_order::getOrdersByStatus("GDSF",$qi,"0");
    if($orders==null){
        $msql="update lottery_result_gdsf set state='$stateType' WHERE qishu='".$qi."'";
    ...
    foreach($orders as $order){
        ...
        if(in_array($selectBall,array("1","2","3","4","5","6","7","8"))){
            ...
            if(in_array($betContent, array($rs['ball_'.$selectBall],$ds, $dx, $wsdx, $hsds, $fw, $zfb, $season, $wuxing))){
                $win_sign = "1";
                $bet_money_total = $order['win']+$order['fs'];
                $bet_type = "彩票自动结算-彩票中奖";
            }else{
                $win_sign = "0";
                $bet_money_total = $order['fs'];
                $bet_type = "彩票自动结算-彩票反水";
            }
        ...
        if($win_sign == "1" || ($win_sign == "0" && $order['fs']>0 || $win_sign == "2")){
            ...
            $balance=	$assets + $bet_money_total;
            $sql = "INSERT INTO `money_log` (`user_id`,`order_num`,`about`,`update_time`,`type`,`order_value`,`assets`,`balance`)
                        VALUES ('$userid','$datereg','广东十分彩',now(),'$bet_type','$bet_money_total','$assets','$balance');";
            $mysqli->query($sql) or die ("插入金钱记录失败!!!".$order['sub_id']);
        }
    }
