...
$html_data = $curl->fetch_url("http://kaijiang.aicai.com/open/difangIssueDetailByKc.do?gameIndex=311&province=guangxi");
	...
	if(strlen($qishu)>0){
		...
		if($tcou==0){
			$sql = "insert into lottery_result_gxsf(qishu,create_time,datetime,ball_1,ball_2,ball_3,ball_4,ball_5) values ('".$qishu."','".$c_time."','".$time."','".$hm1."','".$hm2."','".$hm3."','".$hm4."','".$hm5."')";
		$mysqli->query($sql);
		}
	}
	}}
...
<table width="100%"border="0" cellpadding="0" cellspacing="0">
  <tr> 
    <td align="left">
      ...
      广西快乐十分(<?=$qishu?>期:<?="$hm1,$hm2,$hm3,$hm4,$hm5"?>):
      <span id="timeinfo"></span>
      </td>
  </tr>
</table>
<iframe src="./Js_gxsf.php?qi=<?=$qishu?>&jsType=0&s_time=<?=$jstime?>" frameborder="0" scrolling="no" height="0" width="0"></iframe>
</body>
</html>