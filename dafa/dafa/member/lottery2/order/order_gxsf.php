...
//广西十分彩
$ball_name['name']		= '广西十分彩';
$ball_name['qiu_1']		= '第一球';
$ball_name['qiu_2']		= '第二球';
$ball_name['qiu_3']		= '第三球';
$ball_name['qiu_4']		= '第四球';
$ball_name['qiu_5']		= '第五球';
$ball_name['qiu_6']		= '总和龙虎和';
$ball_name['qiu_7']		= '前三';
$ball_name['qiu_8']		= '中三';
$ball_name['qiu_9']		= '后三';
$ball_name['ball_1'] 	= '1';
...
$bet_money_total = 0;
$win_money_total = 0;
$sql_gxsf="select g.gxsf_lower_bet,g.gxsf_max_bet from user_group g,user_list u where u.user_id='".$_SESSION["userid"]."' and g.group_id=u.group_id limit 0,1";
$query_gxsf = $mysqli->query($sql_gxsf);
$row_gxsf=	$query_gxsf->fetch_array();
for ($i = 0; $i < count($datas); $i++){
    $bet_money_temp = $datas[''.$names[$i].''];
	if(intval($bet_money_temp)<$row_gxsf['gxsf_lower_bet']){
		echo '<script type="text/javascript">alert("该彩票有单注最低金额：'.$row_gxsf['gxsf_lower_bet'].'元!!!");history.go(-1);;</script>';
		exit;
	}
	if(intval($bet_money_temp)>$row_gxsf['gxsf_max_bet']){
		echo '<script type="text/javascript">alert("该彩票有单注最高金额：'.$row_gxsf['gxsf_max_bet'].'元!!!");history.go(-1);;</script>';
		exit;
	}
...
$sql	=	"insert into order_lottery(user_id,Gtype,rtype_str,rtype,bet_info,bet_money,win,lottery_number,bet_time)
                    values ('$userid','$gType','快速-广西十分彩','777','bet_info','$bet_money_total','0','$lottery_number','$bj_time_now')"; //新增一个投注项
$mysqli->query($sql);
...
$sql		= "select * from odds_lottery
            where lottery_type='广西十分彩' and sub_type in('正码和特别号','总和龙虎和','顺子杂六')
            order by id asc";
$query		= $mysqli->query($sql);
$s 			= 1;
while($row = $query->fetch_array()){
...
for ($i = 0; $i < count($datas); $i++){
    ...
    $money	= $datas[''.$names[$i].''];
    $odds = $rs[$qiu[1]]['h'.$qiu[2]];
    if($qiu[1]<6 && $qiu[2]>21){
        $odds = $rs[$qiu[1]]['h'.($qiu[2]+7)];
    }
