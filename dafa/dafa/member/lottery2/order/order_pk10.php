...
$bet_money_total = 0;
$win_money_total = 0;
$sql_bjpk="select g.bjpk_lower_bet,g.bjpk_max_bet from user_group g,user_list u where u.user_id='".$_SESSION["userid"]."' and g.group_id=u.group_id limit 0,1";
$query_bjpk = $mysqli->query($sql_bjpk);
$row_bjpk=	$query_bjpk->fetch_array();
for ($i = 0; $i < count($datas); $i++){
    $bet_money_temp = $datas[''.$names[$i].''];
	if(intval($bet_money_temp)<$row_bjpk['bjpk_lower_bet']){
		echo '<script type="text/javascript">alert("该彩票有单注最低金额：'.$row_bjpk['bjpk_lower_bet'].'元!!!");history.go(-1);;</script>';
		exit;
	}
	if(intval($bet_money_temp)>$row_bjpk['bjpk_max_bet']){
		echo '<script type="text/javascript">alert("该彩票有单注最高金额：'.$row_bjpk['bjpk_max_bet'].'元!!!");history.go(-1);;</script>';
		exit;
	}
