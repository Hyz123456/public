...
<div class="round-table" id="aHeight">
    ...
    <table width="100%" border="0" cellpadding="5" cellspacing="1" class="font12" style="margin-top:2px;" bgcolor="#798EB9">
        <tr  class="title_tr" style="background-color:#3C4D82; color:#FFF">
			...
			<td align="center" colspan="3"><strong>冠亚军和/开奖结果</strong></td>
        </tr>
        <?php
    ...
    FROM lottery_result_bjpk WHERE DATE_FORMAT(datetime,'%Y-%m-%d') = '$query_time'";
        ...
        while($rows = $query->fetch_array()){
            ...
            <tr   class="R_tr" align="center" onMouseOver="this.style.backgroundColor='<?=$over?>'" onMouseOut="this.style.backgroundColor='<?=$out?>'" style="background-color:<?=$color?>; line-height:20px;">
                ...
                ?>
                    <td  colspan="3"><?=Pk10_Auto_quick($hm,1)?> / <span>和</span> / <span>和</span> </td>
                <?
                }else{
                ?>
                    <td  colspan="3"><?=Pk10_Auto_quick($hm,1)?> / <?=(Pk10_Auto_quick($hm,2)=="大"?"<span style=\"color: red;\">".Pk10_Auto_quick($hm,2)."</span>":Pk10_Auto_quick($hm,2))?> / <?=(Pk10_Auto_quick($hm,3)=="双"?"<span style=\"color: red;\">".Pk10_Auto_quick($hm,3)."</span>":Pk10_Auto_quick($hm,3))?></td>
                <?
                }?>
              </tr>
			
			 <tr    class="swidth" style=" line-height:20px;background:#fff;">
