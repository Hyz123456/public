...
function updateOpenTime(){
    $.post("class/odds_jx.php", function(data){
...
function loadinfo(){
    $.post("class/odds_jx.php", function(data)
    {
        if(data.opentime>0){
            ...
            $.jBox.alert('当前彩票已经封盘，请稍后再进行下注！<br><br>江西时时彩开盘时间为：每日09:00 - 次日02:00', '提示');
            return false;
        }
    }, "json");
}
...
//更新开奖号码
function auto(ball){
    $.post("class/auto_jx.php", {ball : ball}, function(data)
    {
