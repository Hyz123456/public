...
$sql	=	"insert into order_lottery(user_id,Gtype,rtype_str,rtype,bet_info,bet_money,win,lottery_number,bet_time,order_num)
                    values ('$userid','$gType','快速-排列三','748','bet_info','$bet_money_total','0','$lottery_number','$bj_time_now','')"; //新增一个投注项
...
$sql		= "select * from odds_lottery_normal
            where lottery_type='排列三' and sub_type in('佰定位','拾定位','个定位','总和龙虎和','3连','跨度')
            order by id asc";
$query		= $mysqli->query($sql);
...
for ($i = 0; $i < count($datas); $i++){
    ...
    if($qiu[1]<4 && $qiu[2]>10){
        ...
        $sql_sub		= "select * from odds_lottery_normal
            where lottery_type='排列三' and sub_type='两面' ";
    ...
    $sql	=	"insert into order_lottery_sub (order_num,quick_type,number,bet_rate,bet_money,win,fs,balance,order_sub_num)
