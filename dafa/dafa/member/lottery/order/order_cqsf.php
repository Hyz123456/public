...
$_GET["game"] = "CQSF";
$gType = "CQSF";
include_once($C_Patch."/pt/mem/ajax/source_cqsf.php");
...
$bet_money_total = 0;
$win_money_total = 0;
$sql_cqsf="select g.cqsf_lower_bet,g.cqsf_max_bet from user_group g,user_list u where u.user_id='".$_SESSION["userid"]."' and g.group_id=u.group_id limit 0,1";
$query_cqsf = $mysqli->query($sql_cqsf);
$row_cqsf   =	$query_cqsf->fetch_array();
for ($i = 0; $i < count($datas); $i++){
    $bet_money_temp = $datas[''.$names[$i].''];
	if(intval($bet_money_temp)<$row_cqsf['cqsf_lower_bet']){
		echo '<script type="text/javascript">alert("该彩票有单注最低金额：'.$row_cqsf['cqsf_lower_bet'].'元!!!");history.go(-1);;</script>';
		exit;
	}
	if(intval($bet_money_temp)>$row_cqsf['cqsf_max_bet']){
		echo '<script type="text/javascript">alert("该彩票有单注最高金额：'.$row_cqsf['cqsf_max_bet'].'元!!!");history.go(-1);;</script>';
		exit;
	}
...
$sql	=	"insert into order_lottery(user_id,Gtype,rtype_str,rtype,bet_info,bet_money,win,lottery_number,bet_time)
                    values ('$userid','$gType','快速-重庆快乐十分','791','bet_info','$bet_money_total','0','$lottery_number','$bj_time_now')"; //新增一个投注项
$mysqli->query($sql);
...
$sql_sub		= "select * from odds_lottery
            where lottery_type='重庆十分彩' and sub_type='方位中发白' order by id asc";
...
$sql		= "select * from odds_lottery
            where lottery_type='重庆十分彩' and (sub_type='正码和特别号' or sub_type='总和龙虎') order by id asc";
$query		= $mysqli->query($sql);
$s 			= 1;
$list 		= array();
