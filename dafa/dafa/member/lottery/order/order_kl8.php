...
$x2x5money=0;
$select_number = 0;
$sql_bjkn="select g.bjkn_lower_bet,g.bjkn_max_bet from user_group g,user_list u where u.user_id='".$_SESSION["userid"]."' and g.group_id=u.group_id limit 0,1";
$query_bjkn = $mysqli->query($sql_bjkn);
$row_bjkn=	$query_bjkn->fetch_array();
for ($i = 0; $i < count($datas); $i++){
	...
	if(intval($bet_money_temp)<$row_bjkn['bjkn_lower_bet']){
		echo '<script type="text/javascript">alert("该彩票有单注最低金额：'.$row_bjkn['bjkn_lower_bet'].'元!!!");history.go(-1);;</script>';
		exit;
	}
	if(intval($bet_money_temp)>$row_bjkn['bjkn_max_bet']){
		echo '<script type="text/javascript">alert("该彩票有单注最高金额：'.$row_bjkn['bjkn_max_bet'].'元!!!");history.go(-1);;</script>';
		exit;
	}
