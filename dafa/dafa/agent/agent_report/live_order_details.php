            ...
            <td valign="top">
                ...
                <table width="100%" border="0" cellpadding="5" cellspacing="1" class="font12" style="margin-top:5px;" bgcolor="#798EB9">
                    <tr style="background-color:#3C4D82; color:#FFF">
                        <td height="25" align="center"><strong>真人类别</strong></td>
                        ...
                        <td height="25" align="center"><strong>赢取金额</strong></td>
                        <td align="center"><strong>投注时间</strong></td>
                        <td align="center"><strong>主账号</strong></td>
                        <td align="center"><strong>真人账号</strong></td>
                    </tr>
                    <?php
                    ...
                    $sql	=	"select lo.id  FROM live_user l,live_order lo
                                where l.live_username=lo.live_username";
                    if($inUserString != "") $sql .= " AND l.user_id IN $inUserString";
                    if($s_time) $sql.=" and lo.order_time>='".$s_time." 00:00:00'";
                    if($e_time) $sql.=" and lo.order_time<='".$e_time." 23:59:59'";
                    ...
                    if($bid){
                        $bid	=	rtrim($bid,',');
                        $sql	=	"SELECT lo.*, u.user_name
                                      FROM live_user l,live_order lo,user_list u
                                      WHERE lo.id in($bid) AND l.live_username=lo.live_username AND l.user_id=u.user_id
                                      order by lo.id desc";
                        $query	=	$mysqli->query($sql);
                        ...
                        while ($rows = $query->fetch_array()) {
                            ...
                            <tr align="center" onMouseOver="this.style.backgroundColor='<?=$over?>'" onMouseOut="this.style.backgroundColor='<?=$out?>'" style="background-color:<?=$color?>; line-height:20px;">
                                <td height="40" align="center" valign="middle"><?=$rows['game_type']?></td>
                                <td align="center" valign="middle"><?=$rows['order_num']?></td>
                                <td align="center" valign="middle"><?=$rows['live_type']?></td>
                                <td align="center" valign="middle"><?=$rows['bet_info']?></td>
                                <td align="center" valign="middle"><?=$rows['bet_money']?></td>
                                <td align="center" valign="middle"><?=$rows['live_win']?></td>
                                <td align="center" valign="middle"><?=$rows['order_time']?></td>
                    ...
                    <tr style="background-color:#FFFFFF;">
                        <td colspan="12" align="center" valign="middle">当前页总投注金额:<?=$t_allmoney?>元 &nbsp;&nbsp;   当前页赢取金额:<?=$t_sy?>元</td>
                    </tr>
                    <tr style="background-color:#FFFFFF;">
