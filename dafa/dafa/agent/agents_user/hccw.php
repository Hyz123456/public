...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#CCCCCC">
  <tr>
    <td height="24" nowrap class="bg_tr"><font ><span class="STYLE2">财务核查</span></font></td>
  </tr>
  <tr>
     ...
     <form name="form1" method="GET" action="hccw.php" >
      <tr>
        ...
        <td width="300">
            ...
            <input name="top_id" type="hidden" id="top_id" value="<?=$_GET['top_id']?>"/>
        </td>
      </tr>
	</form>
    </table></td>
...
<table width="100%" border="1" bgcolor="#FFFFFF" bordercolor="#ccc" cellspacing="0" cellpadding="0" style="border-collapse: collapse; color: #225d9c;" id=editProduct   idth="100%" >
      <tr bgcolor="efe" class="bg_tr" align="center">
        <td width="10%" height="24" ><strong>会员名称</strong></td>
...
if(true){
	...
	$arr_m[2]	=	0;
	$arr_m[3]	=	0;
	$sql		=	"select m.status,m.order_value,m.order_num,m.update_time,m.about,m.id,m.assets,m.balance,m.type from money m left join user_list u on m.user_id=u.user_id where u.top_id=$top_id and (m.type='在线支付' or m.type='用户提款' or m.type='后台充值' or m.type='后台扣款')"; //所有该会员的存款取款记录
    if($time_start){
	...
	$sql	=	"select m.status,m.order_value,m.order_num,m.update_time,m.id,m.assets,m.balance,m.pay_card,u.user_name from money m left join user_list u on m.user_id=u.user_id where u.top_id=$top_id and type='银行汇款'"; //所有该会员的存款取款记录
    if($time_start){
        $stime	 =	$time_start.' 00:00:00';
        $sql	.=	" and m.update_time>='$stime' ";
    }
	...
	while($row = $query->fetch_array()){
        ...
        $arr[$time]['money']	=	$row['order_value'];
        $arr[$time]['user_name']	=	$row['user_name'];
		$arr[$time]['time'] 	=	$row['update_time'];
		$arr[$time]['lsh'] 		=	$row['order_num'];
