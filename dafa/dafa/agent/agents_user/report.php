...
<table width="100%" border="0" align="center" cellpadding="5" cellspacing="1" class="font12" bgcolor="#ccc">
    <tr>
        <td height="24" nowrap class="bg_tr"><font >&nbsp;<span class="STYLE2">报表信息</span></font></td>
    </tr>
...
<table width="100%" border="0" cellpadding="5" cellspacing="1" class="font12" style="margin-top:5px;" bgcolor="#ccc">
    <tr style="background-color:#5a5a5a; color:#FFF">
        ...
        <td style="width: 17%" align="center"><strong>体育流水/体育盈利</strong></td>
        <td style="width: 17%" align="center"><strong>真人流水/真人盈利</strong></td>
        <td style="width: 17%" align="center"><strong>彩票流水/彩票盈利</strong></td>
        <td style="width: 17%" align="center"><strong>六合流水/六合盈利</strong></td>
    ...
    $id = $_GET["id"];
    $sql	=	"SELECT a.id FROM agents_list a WHERE 1=1 AND a.id=$id";
...
//    if($e_time) $sql.=" and o.bet_time<='".$e_time." 23:59:59'";
    ...
    if($bid){
        ...
        $sql_lottery	=	"SELECT SUM(IF(sub.bet_money>0,IF(sub.is_win!=2,sub.bet_money,0),0)) bet_money_total,SUM(IF(sub.is_win=1,sub.win+sub.fs,IF(is_win=0,fs,0))) win_total,u.top_id
                    ...
                    WHERE a.id in($bid) AND a.id=u.top_id AND u.top_id!=0 AND o.status!='0' AND o.status!='3' AND u.user_id=o.user_id AND o.order_num=sub.order_num
                    ";
        ...
        $sql_six	=	"SELECT SUM(IF(sub.bet_money>0,IF(sub.is_win!=2,sub.bet_money,0),0)) bet_money_total,SUM(IF(sub.is_win=1,sub.win+sub.fs,IF(is_win=0,fs,0))) win_total_six,u.top_id
        ...
        $sql_ds	=	"SELECT SUM(IF(k.bet_money>0,k.bet_money,0)) bet_money_total,SUM(IF(k.win>0,k.win,0)+IF(k.fs>0,k.fs,0)) win_total,u.top_id
                    ...
                    WHERE a.id in($bid) AND a.id=u.top_id AND u.top_id!=0 AND k.status!=0 AND k.status!=3 AND k.check!=0 AND u.user_id=k.user_id
                    ";
        ...
        $sql_cg	=	"SELECT SUM(IF(k.bet_money>0,k.bet_money,0)) bet_money_total,SUM(IF(k.win>0,k.win,0)+IF(k.fs>0,k.fs,0)) win_total,u.top_id
                    ...
                    WHERE a.id in($bid) AND a.id=u.top_id AND u.top_id!=0 AND k.status!=0 AND k.status!=3 AND k.check=1 AND u.user_id=k.user_id
                    ";
        ...
        $sql_live	=	"SELECT SUM(IF(lo.bet_money>0,lo.bet_money,0)) bet_money_total,SUM(IF(lo.live_win is not null,lo.live_win,0)) win_total,u.top_id
                    ...
                    WHERE a.id in($bid) AND a.id=u.top_id AND u.top_id!=0 AND l.user_id=u.user_id AND l.live_username=lo.live_username
                    ";
        ...
        foreach($list_main as $key => $value){
            ...
            foreach($list_six as $key2 => $value_six){
                if($value["id"] == $value_six["top_id"]){
                    ...
                    $six_win = ($value_six["bet_money_total"] - $value_six["win_total_six"]);
            ...
            <tr align="center" onMouseOver="this.style.backgroundColor='<?=$over?>'" onMouseOut="this.style.backgroundColor='<?=$out?>'" style="background-color:<?=$color?>; line-height:20px;">
                <td height="40" align="center" valign="middle"><a title="<?=$value['agents_name']?>" style="color: #F37605;" href="agent_child_list.php?id=<?=urldecode($value['id'])?>&amp;s_time=<?=$s_time?>&amp;e_time=<?=$e_time?>&amp;date_month=<?=$date_month?>"><?=$value['agents_name']?></a></td>
