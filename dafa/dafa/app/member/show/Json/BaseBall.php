...
$sql		=	"select id from baseball_match WHERE Match_Type=1 and Match_CoverDate>'".$et_time_now."' AND Match_Date='".date("m-d",$et_time)."' ".$sqlwhere.' order by Match_CoverDate,match_name';
...
if($i == 1){ //未查找到记录
	...
	$sql	=	"select match_name from baseball_match WHERE Match_Type=1 and Match_CoverDate>'".$et_time_now."' AND Match_Date='".date("m-d",$et_time)."' group by match_name";
	$query	=	$mysqli->query($sql);
	$i		=	0;
	$lsm	=	'';
	while($row = $query->fetch_array()){
