...
$sql		=	"select id from bet_match where Match_Type=0 and Match_IsShowbd=1 and Match_CoverDate>'".$et_time_now."' and Match_Hr_Bd21>0 ".$sqlwhere.' order by Match_CoverDate,iPage,match_name,Match_Master,Match_ID,iSn';
...
if($i == 1){ //未查找到记录
	...
	$sql	=	"select match_name from bet_match where Match_Type=0 and Match_IsShowbd=1 and Match_CoverDate>'".$et_time_now."' and Match_Hr_Bd21>0 group by match_name";
	$query	=	$mysqli->query($sql);
	$i		=	0;
	$lsm	=	'';
	while($row = $query->fetch_array()){
	...
	while($rows = $query->fetch_array()){
		...
		$json["db"][$i]["Match_Hr_Bd40"]		=	$rows["Match_Hr_Bd40"];     ///////////     11
		$json["db"][$i]["Match_Hr_Bd41"]		=	$rows["Match_Hr_Bd41"];     ///////////     12
		$json["db"][$i]["Match_Hr_Bd42"]		=	$rows["Match_Hr_Bd42"];     ///////////     13
		$json["db"][$i]["Match_Hr_Bd43"]		=	$rows["Match_Hr_Bd43"];     ///////////     14
		...
		$json["db"][$i]["Match_Hr_Bd44"]		=	$rows["Match_Hr_Bd44"];     ///////////     19
		...
		$json["db"][$i]["Match_Hr_Bdg40"]		=	$rows["Match_Hr_Bdg40"];     ///////////     27
		$json["db"][$i]["Match_Hr_Bdg41"]		=	$rows["Match_Hr_Bdg41"];     ///////////     28
		$json["db"][$i]["Match_Hr_Bdg42"]		=	$rows["Match_Hr_Bdg42"];     ///////////     29
		$json["db"][$i]["Match_Hr_Bdg43"]		=	$rows["Match_Hr_Bdg43"];     ///////////     30
		$i++;
	}
}
$mysqli->close();
