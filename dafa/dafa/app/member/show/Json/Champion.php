...
$sql		=	"select x_id from t_guanjun where match_type=1 and match_coverdate>'".$et_time_now."' and x_result is null ".$sqlwhere.' order by  match_coverdate,match_name,x_id';
...
if($i == 1){ //未查找到记录
	...
	$sql	=	"select x_title from t_guanjun where match_type=1 and match_coverdate>'".$et_time_now."' and x_result is null group by x_title";
	$query	=	$mysqli->query($sql);
	$i		=	0;
	$lsm	=	'';
	while($row = $query->fetch_array()){
