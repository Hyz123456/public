...
$sql		=	"select id from other_match WHERE Match_CoverDate>'".$et_time_now."' AND Match_Date<>'".date("m-d",$et_time)."' ".$sqlwhere.'  order by Match_CoverDate,iPage,iSn,Match_ID,match_name,Match_Master';
...
if($i == 1){ //未查找到记录
	...
	$sql	=	"select match_name from other_match WHERE Match_Type!=3 AND Match_CoverDate>'".$et_time_now."' AND Match_Date<>'".date("m-d",$et_time)."' group by match_name";
	$query	=	$mysqli->query($sql);
	$i		=	0;
	$lsm	=	'';
	while($row = $query->fetch_array()){
	...
	while($rows = $query->fetch_array()){ 
		if($rows["Match_Ho"]==0 && $rows["Match_DxGG"]==0 && $rows["Match_DsDpl"]==0){
			...
			$json["db"][$i]["Match_BzM"]		=	is_null($rows["Match_BzM"])?0:$rows["Match_BzM"];
			$json["db"][$i]["Match_BzG"]		=	is_null($rows["Match_BzG"])?0:$rows["Match_BzG"];
			$json["db"][$i]["Match_BzH"]		=	is_null($rows["Match_BzH"])?0:$rows["Match_BzH"];
		}
		$i++;
	}
}
