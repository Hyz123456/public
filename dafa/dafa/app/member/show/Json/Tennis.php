...
$sql		=	"select id from tennis_Match WHERE Match_Type=1 and Match_CoverDate>'".$et_time_now."' AND Match_Date='".date("m-d",$et_time)."' ".$sqlwhere." order by Match_CoverDate,Match_Name";
...
if($i == 1){ //未查找到记录
	...
	$sql	=	"select match_name from tennis_Match WHERE Match_Type=1 and Match_CoverDate>'".$et_time_now."' AND Match_Date='".date("m-d",$et_time)."' group by match_name";
	$query	=	$mysqli->query($sql);
	$i		=	0;
	$lsm	=	'';
	while($row = $query->fetch_array()){
