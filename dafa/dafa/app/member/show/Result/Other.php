...
<div class="result_title"><span>其他结果 >></span><?php for($i=0;$i<7;$i++){ $d	=	date('Y-m-d',$et_time-$i*86400);$dd	=	date('m-d',$et_time-$i*86400);?><li<?= $d==$date ? ' class="i"' : ''?>><?= $d==$date ? ' ' : '<a href="?ymd='.$d.'">'?><?=$dd?><?= $d==$date ? '' : '</a>'?></li><?php }?></div>
</div>
...
<?php
$sql	=	"select Match_MatchTime, Match_Type,match_name,match_master,match_guest,MB_Inball,TG_Inball,MB_Inball_HR,TG_Inball_HR from other_match where match_Date='".date('m-d',strtotime($date))."' and (MB_Inball is not null or MB_Inball_HR is not NULL) and match_js=1 order by Match_CoverDate,iPage,iSn,Match_ID,match_name,Match_Master ";
