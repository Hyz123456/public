...
$sql	=	"select id,order_value,order_num,update_time,user_id,assets,status,about from money where `user_id`='".$_SESSION["userid"]."' and type='用户提款' and `update_time`>='$get_time' order by id desc";
...
if($cunkuan_list && count($cunkuan_list)>0){
    foreach ($cunkuan_list as $key =>$cunkuan_info) {
        ...
        if($cunkuan_info["status"]=="成功"){
            $sum += abs($cunkuan_info["order_value"]);
...
<td style="text-align:center;">'.abs($cunkuan_info["order_value"]).'</td>
<td style="text-align:center;">'.$statusString.'</td>
<td style="text-align:center;">'.$cunkuan_info["about"].'</td>
</tr>';
    }
    $subPage .= '<tr ><td colspan="5" style="text-align:center;">本页取款成功总金额：'.$sum.'</td></tr>';
}else{
    $subPage = '<td colspan="5" style="text-align:center;">暂时没有下注信息。</td>';
}
?>
    ...
    <div class="Hyzx-body">                
        ...
        <div class="Hyzx-content">
			...
			<input type="text" value="<?=date('Y-m-d',strtotime('-6 day'))?>" style="width: 85px;background-color: white;" disabled="disabled"/> ~
                        ...
                        <input type="button" class="flt Hyzx-btn" value="取款记录" onclick="chgType('chakan_qukuan');" onmouseover="mover(this);" onmouseout="mout(this);"  style="background: #875e5a;"/>
            ...
            <table class="Hyzx-table mt10 MMain">
                <thead>
            <tr>
                <th width="28%" align="center">取款流水号</th>
                <th width="22%" align="center">取款时间</th>
                <th width="15%" align="center">取款金额</th>
                <th width="15%" align="center">取款状态</th>
