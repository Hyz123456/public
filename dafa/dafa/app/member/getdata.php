...
if($web_site['close'] == 1) {
	...
	if( $userid>0){ //已登陆
		...
		/*$sql = "select ag_money,bb_money,mg_money,ab_money,na_money,pt_money from user_list where user_id='$userid' ";
		$query	=	$mysqli->query($sql);
        $rs    =	$query->fetch_array();
		$ag_money	=	sprintf("%.2f",($rs['ag_money']));
		$bb_money	=	sprintf("%.2f",($rs['bb_money']));
		$mg_money	=	sprintf("%.2f",($rs['mg_money']));
		$ab_money	=	sprintf("%.2f",($rs['ab_money']));
		$na_money	=	sprintf("%.2f",($rs['na_money']));
		$pt_money	=	sprintf("%.2f",($rs['pt_money']));
		$json['ag_money'] = $ag_money;
		$json['bb_money'] = $bb_money;
		$json['mg_money'] = $mg_money;
		$json['ab_money'] = $ab_money;
		$json['na_money'] = $na_money;
		$json['pt_money'] = $pt_money;
