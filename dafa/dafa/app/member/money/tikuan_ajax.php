...
if(strtoupper($_SESSION['username'])!=strtoupper($t["user_name"])){
    echo "缓存过期，请退出账号，然后关闭浏览器并清空缓存，最后重新登录做提款操作。";
    exit;
}
...
//	}
	...
	if($rows){
		...
		try{
            ...
            $sql  =   "INSERT INTO `money_log` (`user_id`,`order_num`,`about`,`update_time`,`type`,`order_value`,`assets`,`balance`) VALUES ('$uid','$order','$about',now(),'用户提款','$pay_value_log','$assets',$assets+$pay_value);";
			$mysqli->query($sql);
			...
			if($q1==1 && $q2==1 && $q3==1){
				$mysqli->commit(); //事务提交
//				echo '提款申请已经提交，等待财务人员给您转账。','../user/cha_qukuan.php';
                echo "chakan_qukuan";
                exit;
			}else{
...
<div id="waikuan"  style="width:735px;">
  <div id="top_2">
	 ...
	 <form onsubmit="return  check_submit()" action="/app/member/money/tikuan.php?action=tikuan" method="post" name="form1">
       <tr>
       ...
       <tr>
         ...
         <td height="24"><span class="fudong_9">
           <input name="submit" type="submit" class="anniu_001" id="btn" value="提交"/>
         </span></td>
       </tr>
     </form>
    </table></td>
