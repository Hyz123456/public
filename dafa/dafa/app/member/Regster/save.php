...
if($_POST["username"] && $_POST["password"])
{
	...
	$qq_num		=	strip_tags($_POST["qq_num"])?strip_tags($_POST["qq_num"]):0;
	$email		=	strip_tags($_POST["email"]);
	$top_id		=	strip_tags($_POST["agent_id"]);
	if($top_id==""){
		if($_POST['agent_name']){
			$sql="select id from agents_list where agents_name='".$_POST['agent_name']."'";
			$query=$mysqli->query($sql);
			$row=$query->fetch_array();
			if($row)
				$top_id=$row['id'];
			else{
				echo "<script>alert('抱歉,不存在该代理!!!');history.go(-1);</script>";
				exit;
			}
		}
		else
	...
	$loginaddress=	"";
        $logouttime     =   "2000-01-01 00:00:00";
    ...
    $query		=	$mysqli->query($sql);
    $rs			=	$query->fetch_array();
    if($rs['today_count'] && $rs['today_count']>20){
...
try{
	...
	if($q1 == 1 ){
        ...
        /*if(!$a){
            echo "<script>alert('1111');</script>";exit;
        }*/
		header("Content-type: text/html; charset=utf-8");
		echo '<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />';
		echo "<script>alert(\"恭喜您，注册成功。\");</script>";

			header("Location: rule.php?username=".$user_name."&userpass=".$_POST["password"]."");
			exit();
