...
class report_live
{
	static function getZhenRenCount($dayStart,$dayEnd,$inUserString){
		$oneDayStart = $dayStart.' 00:00:00';
        $oneDayEnd = $dayEnd.' 23:59:59';
		global $mysqli;
		$sql	=	"SELECT count(about) as bishu,sum(order_value) as zonge, about  FROM money_log where type='真人转账' AND update_time>= '".$oneDayStart."' AND update_time <='".$oneDayEnd."' ";
		if($inUserString)
			$sql.=" and user_id in ".$inUserString;
		$sql.=" GROUP BY about";
        return $sql;
