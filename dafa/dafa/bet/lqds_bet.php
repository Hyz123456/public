...
<meta http-equiv="Cache-Control" content="max-age=0" />
	...
	<card title="篮球单式">
		<onevent type="ontimer">
			<go href="bet_lqds.php?matchname=<?=urlencode($name)?>"/>
		</onevent>
		<timer value="200"/>
...
?>
		...
		<br/>
		<a href="lqds_bet.php?matchname=<?=urlencode($name)?>&amp;type=<?=$type?>&amp;matchid=<?=$id?>&amp;<?=rand()?>" title="刷新">刷新</a> <anchor title="交易">交易<go href="bet.php" method="post"><postfield name="money" value="$(money)" /><postfield name="type" value="<?=$type?>" /><postfield name="pl" value="<?=double_format($row[$type])?>" /><postfield name="id" value="<?=$row['Match_ID']?>" /><postfield name="matchname" value="<?=$name?>" /><postfield name="sort" value="篮球单式" /><?=$pk?></go></anchor><br/>最低限额：<?=$group_db['sports_lower_bet']?><br/>

		<a href="bet_lqds.php?matchname=<?=urlencode($name)?>" title="继续交易">继续交易</a><br/>
