...
<link href="/bet/css/commonbet.css" rel="stylesheet" type="text/css">
	...
	<body title="篮球单节">
			<section class="nav_list">
				<a href="bet_lqdj.php?matchname=<?=urlencode($name)?>" title="刷新">刷新<em></em></a>
...
?>			
			...
			<li>
				...
				echo '<a href="lqdj_bet.php?matchname='.urlencode($name).'&amp;matchid='.$row['ID'].'&amp;type=Match_Ho" title="'.double_format($row["Match_Ho"]).'">'.double_format($row["Match_Ho"]).'</a>';
				?>
				</div>
			<?php
					?>
			...
			<?php
				...
				echo '<a href="lqdj_bet.php?matchname='.urlencode($name).'&amp;matchid='.$row['ID'].'&amp;type=Match_Ao" title="'.double_format($row["Match_Ao"]).'">'.double_format($row["Match_Ao"]).'</a>';
				?>
				</div>
				<?php
				?>
		...
		<?php
			...
			<li>
				<div class="twos">
					大(<?=$row["Match_DxGG"]?>) 
					<a href="lqdj_bet.php?matchname=<?=urlencode($name)?>&amp;matchid=<?=$row['ID']?>&amp;type=Match_DxDpl" title="<?=double_format($row["Match_DxDpl"])?>"><?=double_format($row["Match_DxDpl"])?>
					</a>
				</div>
				<div class="twos">
					小(<?=$row["Match_DxGG"]?>) 
					<a href="lqdj_bet.php?matchname=<?=urlencode($name)?>&amp;matchid=<?=$row['ID']?>&amp;type=Match_DxXpl" title="<?=double_format($row["Match_DxXpl"])?>"><?=double_format($row["Match_DxXpl"])?>
					</a>
				</div>
			</li>
			<?php
			...
			<li>
				<div class="twos">
					单 <a href="lqdj_bet.php?matchname=<?=urlencode($name)?>&amp;matchid=<?=$row['ID']?>&amp;type=Match_DsDpl" title="<?=double_format($row["Match_DsDpl"])?>"><?=double_format($row["Match_DsDpl"])?></a>
				</div>
				<div class="twos">
					双 <a href="lqdj_bet.php?matchname=<?=urlencode($name)?>&amp;matchid=<?=$row['ID']?>&amp;type=Match_DsSpl" title="<?=double_format($row["Match_DsSpl"])?>"><?=double_format($row["Match_DsSpl"])?></a>
				</div>
			</li>
